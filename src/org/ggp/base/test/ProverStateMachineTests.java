...
public class ProverStateMachineTests extends Assert
{
  ...
  {
    List<Gdl> ticTacToeDesc = new TestGameRepository().getGame("ticTacToe")
        .getRules();
    ...
    /*
     * Move m23 = new Move(GdlPool.getRelation(PLAY, new GdlTerm[] {C2, C3,
     * O})); try { sm.getNextState(state, Arrays.asList(new Move[] {noop,
     * m23})); fail(
     * "Should throw an exception when trying to transition from a terminal state"
     * ); } catch(TransitionDefinitionException e) { //Expected }
     */
  }
  ...
  {
    List<Gdl> desc = new TestGameRepository().getGame("test_case_1a")
        .getRules();
    sm.initialize(desc);
    MachineState state = sm.getInitialState();
  ...
  {
    List<Gdl> desc = new TestGameRepository().getGame("test_case_3c")
        .getRules();
    sm.initialize(desc);
    MachineState state = sm.getInitialState();
  ...
  {
    List<Gdl> desc = new TestGameRepository().getGame("test_case_5a")
        .getRules();
    sm.initialize(desc);
    MachineState state = sm.getInitialState();
    Role you = new Role(GdlPool.getConstant("you"));
  ...
  {
    List<Gdl> desc = new TestGameRepository().getGame("test_case_5b")
        .getRules();
    sm.initialize(desc);
    MachineState state = sm.getInitialState();
    Role you = new Role(GdlPool.getConstant("you"));
    ...
    state = sm.getNextState(state,
                            Collections.singletonList(move("draw 1 1 1 2")));
    assertTrue(sm.isTerminal(state));
  }
  ...
  {
    List<Gdl> desc = new TestGameRepository().getGame("test_case_5c")
        .getRules();
    sm.initialize(desc);
    MachineState state = sm.getInitialState();
    Role you = new Role(GdlPool.getConstant("you"));
