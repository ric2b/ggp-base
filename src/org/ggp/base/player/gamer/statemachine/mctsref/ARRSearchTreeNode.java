    ...
    else
    {
      ...
      for(int i = 0; i < children.length; i++)
      {
        ...
        if ( child.numVisits > 0 && child.scoreVector[choosingRole] > pivot )
        {
          ...
          pivotStdDeviationEstimate = ((ARRSearchTreeNode)child).getStdDeviationEstimate();
          pivotChild = child;
        }
      }
...
//      }
      ...
      for(int i = 0; i < children.length; i++)
      {
        ...
        if ( child.numVisits > 0 )
        {
          ...
          double childStdDeviationEstimate = ((ARRSearchTreeNode)child).getStdDeviationEstimate();
  ...
  private void updateScoreNormalDistAllCompare(SearchTreeNode xiChild, double[] xiPlayoutResult)
  {
    ...
    else
    {
      ...
      for(int i = 0; i < children.length; i++)
      {
        ARRSearchTreeNode child = (ARRSearchTreeNode)children[i];
        ...
        if ( child.numVisits > 0 )
        {
          ...
          for(int j = 0; j < children.length; j++)
          {
            if ( j != i )
            {
              ...
              double jExceedsI = cumulativeProb(scoreDiff, child.getStdDeviationEstimate() + ((ARRSearchTreeNode)children[j]).getStdDeviationEstimate());
