...
public class Qixote extends SampleGamer
{
  ...
  private void processCompletedRollouts(boolean block)
    ...
    do
    {
      ...
      if (request != null)
      {
        ...
        if (request.node.seq == node.seq && !node.complete)
        {
          ...
          node.updateStats(request.score,
                           request.sampleSize,
                           request.path,
                           false);
  ...
  private class TreeNode
  {
    ...
    private void adjustPropagatedContribution(double delta,
                                              TreeNode from,
                                              List<TreeNode> path)
    {
      if (Math.abs(delta) > epsilon && (path == null || !path.contains(this)))
      {
        ...
        if (from != this)
        {
          ...
          newAverageScore = (averageScore * numVisits + (averageScore + delta) *
                                                        childVisits) /
                            (numVisits + childVisits);
          newAverageSquaredScore = (averageSquaredScore * numVisits + (averageScore + delta) *
                                                                      (averageScore + delta) *
                                                                      childVisits) /
                                   (numVisits + childVisits);
        }
        ...
        {
          parent.adjustPropagatedContribution(newAverageScore - averageScore,
                                              this,
                                              path);
        }
    ...
    private void processCompletion()
    {
      ...
      if (children != null)
      {
        ...
        for (TreeNodeRef cr : children)
        {
          ...
          {
            if (!retainUsefulLine || !cr.node.complete ||
                Math.abs(averageScore + cr.node.averageScore - 100) > 0.5)
            {
              ...
              if (cr.node.seq == cr.seq)
              {
                if (cr.node.parents.size() != 0)
                {
                  ...
                  else
                  {
                    ...
                    cr.node.numVisits = (cr.node.numVisits * numRemainingParents) /
                                        (numRemainingParents + 1);
                  }
                }
              }
            }
        ...
        else
        {
          if (leastLikelyWinner != -1)
          {
            ...
            if (cr.seq == c.seq)
            {
              ...
              {
                uctValue = -cr.node.exploitationUCT() -
                           explorationUCT(childVisitCount);
              }
    ...
    {
      double varianceBound = (averageSquaredScore - averageScore *
                                                    averageScore) /
                             10000 +
                             Math.sqrt(2 *
                                       Math.log(Math.max(numVisits,
                                                         numChildVisits) + 1) /
                                       numChildVisits);
      return explorationBias *
             Math.sqrt(2 * Math.min(0.5, varianceBound) *
                       Math.log(Math.max(numVisits, numChildVisits) + 1) /
                       numChildVisits);
    ...
    public double rollOut(List<TreeNode> path)
        throws TransitionDefinitionException, MoveDefinitionException,
        GoalDefinitionException
    {
      if (complete)
        ...
        GoalDefinitionException
    {
      ...
      if (children == null || onPath)
      {
        ...
        if (!complete)
        {
          double playerNormalizedValue = (ourMove == null ? 100 - value
                                                         : value);
          ...
          double newAverageScore = (averageScore * numVisits + weightedScore) /
                                   (numVisits + adjustedSampleSize);
          averageSquaredScore = (averageSquaredScore * numVisits + playerNormalizedValue *
                                                                   playerNormalizedValue *
                                                                   adjustedSampleSize) /
                                (numVisits + adjustedSampleSize);
      ...
      GoalDefinitionException
  {
    ...
    while (System.currentTimeMillis() < finishBy && !root.complete)
    {
      ...
      if (numRolloutThreads > 0 &&
          loopCount % (maxOutstandingRolloutRequests * 16) == 0)
      {
      ...
      if (System.currentTimeMillis() > updateAt)
      {
        ...
        notifyObservers(new WorkingResponseSelectedEvent(root
            .getBestMove(false).toString()));
      }
    ...
    //validateAll();
    List<Move> moves = underlyingStateMachine.getLegalMoves(currentState,
                                                            ourRole);
    ...
    System.out.println("Average rollout utilization: " +
                       averageRolloutProcessorUtilization);
