...
public class TestMinimaxGamer extends SampleGamer {
	...
	private void determinePropositionWeights(long timeout, int sampleDepth) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
		...
		{  
    	    int theScore = underlyingStateMachine.getDepthChargeResult(getCurrentState(), getRole(), sampleDepth, sampleState, null);
    	    ...
    	    {
    	    	if ( s != underlyingStateMachine.getXSentence() && s != underlyingStateMachine.getOSentence())
    	    	{
		...
		if ( maxScore > minScore )
		{
			for(GdlSentence s : propositionWeights.keySet())
			{
				...
				int normalizedValue = ((rawValue-minScore)*100)/(maxScore-minScore);
	...
	private Move selectPuzzleMove(List<Move> moves, long timeout) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
		...
		{
			Set<MachineState> goalStates = underlyingStateMachine.findGoalStates(getRole(), 99, 100, 20);
		...
		nodesExamined = 0;
		HeuristicType searchType = (terminalState.getContents() == null ? HeuristicType.HEURISTIC_TYPE_GOAL_VALUE : HeuristicType.HEURISTIC_TYPE_GOAL_PROXIMITY);
		...
		if ( goalState == null && bestScore < bestScoreGoaled )
		{
			...
			{
				considered.clear();
				int score = minEval(getCurrentState(), move, -1, 100000, 6, nextExploreType);
				considered.clear();
	...
	private int stateSetDistance(MachineState queriedState, Set<MachineState> targetStates)
	{
		...
		try {
			...
			else
			{
				...
				case HEURISTIC_TYPE_EXPLORE_AWAY:
					...
					result = (4*(100*matchCount)/targetPropositions.size() + (100 - stateDistance(state, terminalState)))/5;
				...
				case HEURISTIC_TYPE_EXPLORE_NEW:
					...
					result = (4*weightedExplorationResult + (100 - stateDistance(state, terminalState)))/5;
					break;
				}
			}
	...
	private int minEval(MachineState state, Move move, int alpha, int beta, int depth, HeuristicType searchType) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
	...
	private int maxEval(MachineState state, int alpha, int beta, int depth, HeuristicType searchType) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException
	{
		int result;
		...
		if ( getStateMachine().isTerminal(state))
		{
			...
			if ( isPuzzle )
			{
				if ( result > bestScoreGoaled )
				{
					System.out.println("Encountered terminal node, returning score of " + result + " in state " + state);
					goalState = state;
					bestScoreGoaled = result;
				}
				else if ( goalState != null && state.equals(goalState) )
