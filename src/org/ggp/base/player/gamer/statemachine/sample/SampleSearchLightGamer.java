...
/**
 * SampleSearchLightGamer is a simple state-machine-based Gamer. It will, to
 * the best of its ability, never pick a move which will give its opponent a
 * possible one-move win. It will also spend up to two seconds looking for
 * one-move wins it can take. This makes it slightly more challenging than the
 * RandomGamer, while still playing reasonably fast. Essentially, it has a
 * one-move search-light that it shines out, allowing it to avoid moves that
 * are immediately terrible, and also choose moves that are immediately
 * excellent. This approach implicitly assumes that it is playing an
 * alternating-play game, which is not always true. It will play
 * simultaneous-action games less well. It also assumes that it is playing a
...
public final class SampleSearchLightGamer extends StateMachineGamer
{
  ...
  /**
   * Employs a simple sample "Search Light" algorithm. First selects a default
   * legal move. It then iterates through all of the legal moves in random
   * order, updating the current move selection using the following criteria.
   * <ol>
   * <li>If a move produces a 1 step victory (given a random joint action)
   * select it</li>
      ...
      GoalDefinitionException
  {
    ...
    for (Move moveUnderConsideration : movesInRandomOrder)
    {
      ...
      if (System.currentTimeMillis() > start + 2000 && reasonableMoveFound)
        break;
      ...
      MachineState nextState = theMachine
          .getNextState(getCurrentState(), theMachine
              .getRandomJointMove(getCurrentState(),
                                  getRole(),
      ...
      {
        MachineState nextNextState = theMachine.getNextState(nextState,
                                                             jointMove);
