...
public class Sancho extends SampleGamer
{
                                       ...
                                       TreeNode previousNode)
  {
    ...
    for (int i = 0; i < numRoles; i++)
    {
      ...
      if (root.numVisits > 50)
      {
        ...
        {
          heuristicStateValueBuffer[i] = root.averageScores[i] +
                                         (100 - root.averageScores[i]) *
  ...
  private class TreeNode
  {
      ...
      for (int roleIndex = 0; roleIndex < numRoles; roleIndex++)
      {
        if (averageScores[roleIndex] > 99.5)
        {
          if (roleIndex == decidingRoleIndex &&
              (!isSimultaneousMove || roleIndex == 0 || hasSiblinglessParents()))
          {
    ...
    private void checkChildCompletion(boolean checkConsequentialSiblingCompletion)
    {
      ...
      if (allImmediateChildrenComplete && !decidingRoleWin &&
          isSimultaneousMove && decidingRoleIndex == 0)
      {
      ...
      if (allImmediateChildrenComplete || decidingRoleWin)
      {
        ...
        {
          decisiveCompletionRatio[roleIndex] = (decisiveCompletionRatio[roleIndex] *
                                                numCompletionsProcessed + 1) /
                                               (numCompletionsProcessed + 1);
        }
        ...
        GoalDefinitionException, InterruptedException
    {
      ...
      try
      {
        ...
        if (!cur.complete)
        {
          ...
          {
            selected = cur
                .select(selected == null ? null : selected.getEdge(),
                        moveWeights);
            newNode = selected.getChildNode();
            //visited.add(newNode);
            visited.push(selected);
            while (newNode.decidingRoleIndex != numRoles - 1 &&
                   !newNode.complete)
            {
        ...
        GoalDefinitionException
    {
      ...
      try
      {
        if (children == null || trimmedChildren > 0)
        {
          ...
          ForwardDeadReckonLegalMoveSet moves = underlyingStateMachine
              .getLegalMoves(state);
          ...
          while (index < newChildren.length)
          {
            ...
            {
              newState = underlyingStateMachine
                  .getNextState(state, newEdge.jointPartialMove);
            }

            newEdge.child = allocateNode(underlyingStateMachine,
                                         newState,
                                         this).getRef();
            ...
            else
            {
              ...
              if (underlyingStateMachine.isTerminal(newState))
              {
                ...
                for (int i = 0; i < numRoles; i++)
                {
                  newChild.averageScores[i] = ((newChild.averageScores[i] + bonusBuffer[i]) * 100) /
                                              (100 + 2 * (numRoles - 1) *
            ...
            if (newChild.numVisits == 0 && heuristicSampleWeight > 0 &&
                !newChild.isTerminal)
            {
              ...
              for (int i = 0; i < numRoles; i++)
              {
                ...
                heuristicSquaredDeviation += (root.averageScores[i] - heuristicScores[i]) *
                                             (root.averageScores[i] - heuristicScores[i]);
              }
    ...
    private int getNumPieces(ForwardDeadReckonInternalMachineState afterState,
                             int forRole)
    {
      if (!state.equals(afterState) && decidingRoleIndex == forRole &&
          numChoices != null && numChoices[forRole] > 1)
                                  ...
                                  int roleIndex)
    {
      ...
      return explorationBias *
             Math.sqrt(2 *
                       ...
                       numChildVisits) / roleRationality[roleIndex];
    }
        ...
        if (children != null)
        {
          for (TreeEdge edge : children)
          {
            if ((decidingRoleIndex + 1) % numRoles == roleIndex &&
                edge.jointPartialMove[roleIndex].move.equals(move))
            {
    ...
    private double getAverageCousinMoveValue(TreeEdge relativeTo, int roleIndex)
    {
      ...
      else if (cousinMovesCachedFor == null ||
               cousinMovesCachedFor.node != this ||
               cousinMovesCachedFor.seq != seq)
      {
            ...
            if (edge.child.seq == child.seq && child.children != null)
            {
              ...
              for (TreeEdge nephewEdge : child.children)
              {
                ...
                if (nephewEdge.child.seq == nephew.seq)
                {
                  ...
                  MoveScoreInfo accumulatedMoveInfo = cousinMoveCache
                      .get(move);
    ...
    private double exploitationUCT(TreeEdge inboundEdge, int roleIndex)
    {
      ...
      else if (useGoalHeuristic)
      {
        ...
        try
        {
          roleScore = netScore(underlyingStateMachine,
                               inboundEdge.child.node.state);
        }
        catch (GoalDefinitionException e)
        {
        ...
        return inboundEdge.child.node.averageScores[roleIndex] / 100 +
               roleScore /
               (100 * Math.log(inboundEdge.child.node.numVisits + 2));// + averageSquaredScore/20000;
      }
      else
      {
                                                       ...
                                                       int forRoleIndex)
    {
      ...
      else if ((decidingRoleIndex + 1) % numRoles == forRoleIndex &&
    ...
    public Move getBestMove(boolean traceResponses)
    {
      ...
      for (TreeEdge edge : children)
      {
        ...
        double moveScore = (isSimultaneousMove || isMultiPlayer || anyComplete || disableOnelevelMinimax) ? child.averageScores[0]
                                                                                                         : child
                            ...
                            boolean isCompletePseudoRollout)
    {
      ...
      for (int roleIndex = 0; roleIndex < numRoles; roleIndex++)
      {
        ...
        if ((!complete || isSimultaneousMove || isMultiPlayer) &&
            childEdge != null)
        {
  ...
  private double patternMatchValue(ForwardDeadReckonInternalMachineState state)
  {
    ...
    if (patterns != null)
    {
      for (Entry<ForwardDeadReckonInternalMachineState, Integer> pattern : patterns
          .entrySet())
      {
      ...
      GoalDefinitionException
  {
    ...
    explorationBias = 18 / (averageNumTurns + ((maxNumTurns + minNumTurns) / 2 - averageNumTurns) *
    ...
    System.out
        .println("Set moveActionHistoryBias to " + moveActionHistoryBias);
    ...
    if (minNumTurns == maxNumTurns ||
        ((greedyRolloutCost > 8 || stdDevNumTurns < 0.15 * averageNumTurns || underlyingStateMachine.greedyRolloutEffectiveness < underlyingStateMachine.numRolloutDecisionNodeExpansions / 3) && !isPuzzle))
    {
      if (!greedyRolloutsDisabled)
      {
        greedyRolloutsDisabled = true;
            ...
            while (!complete && !stopRequested)
            {
              ...
              else
              {
                synchronized (treeLock)
                {
                  ...
                  if (!complete)
                  {
                    explorationBias = maxExplorationBias -
                                      percentThroughTurn *
