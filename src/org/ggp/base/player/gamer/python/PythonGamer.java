 ...
 * a subclass of PythonGamer that overrides getPythonGamerName() and
 * getPythonGamerModule() to indicate where the Python source code file is.
 * This is the Java stub that refers to the real Python gamer class. 2) Create
 * the Python source code file, in the /src_py/ directory in the root directory
 * for this project. Make sure that the stub points to this class, and that the
 * Python class is a valid subclass of Gamer. For examples where this has
 * already been done, see @PythonRandomGamerStub, which is implemented in
 * Python and hooks into the Java framework using the PythonGamer stub.
 * 
 * @author Sam
 * @author evancox
 */
  ...
  private void lazilyLoadGamerStub()
  {
    if (thePythonGamer == null)
    {
      try
      {
        ...
        interpreter.exec("from " + getPythonGamerModule() + " import " +
                         getPythonGamerName());
