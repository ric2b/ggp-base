                                                       ...
                                                       Observer
{
  ...
  /**
   * @param args
   *          Command line arguments: ProxyGamePlayerClient gamer port
   */
  public static void main(String[] args)
  {
    ...
    List<Class<? extends Gamer>> gamers = Lists
        .newArrayList(ProjectSearcher.GAMERS.getConcreteClasses());
    ...
    {
      GamerLogger
          .logError("Proxy", args[0] +
                             " is not a subclass of gamer.  Valid options are:");
      ...
      try
      {
        ...
        if (request instanceof StopRequest)
        {
          GamerLogger.log("Proxy",
                          "[ProxyClient] Got stop request, shutting down.");
          System.exit(0);
        }
        if (request instanceof AbortRequest)
        {
          GamerLogger.log("Proxy",
                          "[ProxyClient] Got abort request, shutting down.");
          System.exit(0);
        }
      }
      catch (Exception e)
  ...
  public void observe(Event event)
  {
    if (event instanceof WorkingResponseSelectedEvent)
    {
      ...
      ProxyMessage theMessage = new ProxyMessage("WORK:" +
                                                     theWorking.getWorkingResponse(),
                                                 theCode,
                                                 0L);
