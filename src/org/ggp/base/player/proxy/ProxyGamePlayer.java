 ...
 * to the server on time, accepts and stores working moves, and so on. This
 * class is not necessary, unless you are interested in adding another layer of
 * bullet-proofing to your player in preparation for a tournament or for
 * running your player for long periods of time. There are advantages and
 * disadvantages to this approach. The advantages are: 1. Even if the Gamer
 * process stalls, for example due to garbage collection, you will always send
 * a legal move back to the server in time. 2. You can send "working moves" to
 * the proxy, so that if your Gamer process stalls, you can send back your
 * best-guess move from before the stall. The disadvantage is very simple: 1.
 * If the proxy breaks, you can revert to playing extremely poorly even though
 * your real Gamer process is fully functional. The advantages are very
...
public final class ProxyGamePlayer extends Thread implements Subject
{
  ...
  private class ClientManager extends Thread
  {
    ...
    public ClientManager(Thread parentThread)
    {
      ...
      try
      {
        GamerLogger.log("Proxy",
                        "[PROXY] Starting a new proxy client, using gamer " +
                            gamerName + ".");

        theClientProcess = pb.start();
        outConnector = new StreamConnector(theClientProcess.getErrorStream(),
                                           System.err);
        errConnector = new StreamConnector(theClientProcess.getInputStream(),
                                           System.out);
        outConnector.start();
        errConnector.start();
        ...
        theInput = new BufferedReader(new InputStreamReader(theClientConnection
            .getInputStream()));
    ...
    public void sendMessage(ProxyMessage theMessage)
    {
      if (theOutput != null)
      {
        ...
        GamerLogger.log("Proxy", "[PROXY] Wrote message to client: " +
                                 theMessage);
      }
    }
    ...
    public void run()
    {
      while (theInput != null)
      {
        ...
        catch (SocketException se)
        {
          ...
          GamerLogger
              .logError("Proxy",
                        "Shutting down reader as consequence of socket exception. Presumably this is because the gamer client crashed.");
          break;
        }
        catch (Exception e)
        {
      ...
      throws IOException
  {
    ...
    while (listener == null)
    {
      ...
      catch (Exception ex)
      {
        listener = null;
        port++;
        GamerLogger.logError("Proxy", "Failed to start gamer on port: " +
    ...
    GamerLogger.log("Proxy",
                    "[PROXY] Opened client communication socket on port " +
                        clientPort + ".");
  ...
  public void run()
  {
    ...
    while (true)
    {
      try
      {
        ...
        GamerLogger.log("Proxy", "[PROXY] Selected fallback move:" +
                                 latestProxiedResponse);
        ...
        else
        {
          ...
          latestProxiedResponse = latestProxiedResponse
              .replace(theDefaultGamer.getName(), gamerName);
        }
        ...
        if (!receivedClientMove && (request instanceof PlayRequest))
        {
          GamerLogger
              .logError("Proxy",
                        "[PROXY] Did not receive any move information from client for this turn; falling back to first legal move.");
          GamerLogger
              .logError("ExecutiveSummary",
                        "Proxy did not receive any move information from client this turn: used first legal move.");
        }
        ...
        if (needRestart)
        {
          ...
          GamerLogger
              .log("Proxy", "[PROXY] Before collection, using " +
          ...
          GamerLogger.log("Proxy",
                          "[PROXY] After collection, using a non-transient " +
                              usedMemoryInMegs + "mb of memory as proxy.");

          System.out.println("Cleaned up completed match, with a residual " +
                             usedMemoryInMegs + "mb of memory as proxy.");
  ...
  private void proxyProcessRequest(Request theRequest, long receptionTime)
  {
    ...
    try
    {
      if (theRequest instanceof PlayRequest)
      {
        if (theDefaultGamer.getMatch() != null)
        {
          ...
          timeToFinish = receptionTime +
                         theDefaultGamer.getMatch().getPlayClock() * 1000 -
                         PLAY_BUFFER;
        }
        else
        {
      ...
      {
        GamerLogger.startFileLogging(theDefaultGamer.getMatch(),
                                     theDefaultGamer.getRoleName().toString());

        System.out.println("Started playing " +
                           theDefaultGamer.getMatch().getMatchId() + ".");

        // They have this long to metagame
        timeToFinish = receptionTime +
                       theDefaultGamer.getMatch().getStartClock() * 1000 -
                       METAGAME_BUFFER;
        timeToSleep = timeToFinish - System.currentTimeMillis();
        if (timeToSleep > 0)
          Thread.sleep(timeToSleep);
               ...
               theRequest instanceof AbortRequest)
      {
    ...
    GamerLogger.log("Proxy",
                    "[PROXY] Proxy slept for " +
                        (System.currentTimeMillis() - startSleeping) +
                        ", and woke up " +
                        (System.currentTimeMillis() - timeToFinish) +
                        "ms late (started " + (startSleeping - receptionTime) +
                        "ms after receiving message).");
  }
  ...
  private void processClientResponse(ProxyMessage in, Thread toWakeUp)
  {
    ...
    if (!(in.messageCode == currentMoveCode))
    {
      if (currentMoveCode > 0)
        GamerLogger.logError("Proxy", "CODE MISMATCH: " + currentMoveCode +
                                      " vs " + in.messageCode);
      return;
    }
    ...
    if (theirTag.equals("WORK:"))
    {
      ...
      GamerLogger.log("Proxy", "[PROXY] Got latest working move: " +
                               latestProxiedResponse);
      receivedClientMove = true;
    }
    ...
    {
      latestProxiedResponse = theirMessage;
      GamerLogger.log("Proxy", "[PROXY] Got a final move: " +
                               latestProxiedResponse);
      receivedClientMove = true;
      currentMoveCode = 0L;
      toWakeUp.interrupt();
      ...
      while (true)
      {
        try
        {
          ...
          if (inputQueue.remainingCapacity() > 0)
          {
            ...
            GamerLogger
                .log("Proxy",
                     "[PROXY QueueListener] Got incoming message from game server: " +
                         in +
                         ". Added to queue in position " +
                         inputQueue.size() + ".");
          }
          else
          {
            GamerLogger
                .logError("Proxy",
                          "[PROXY QueueListener] Got incoming message from game server: " +
                              in +
                              ". Could not add to queue, because queue is full!");
          }
        }
        catch (Exception e)
        {
