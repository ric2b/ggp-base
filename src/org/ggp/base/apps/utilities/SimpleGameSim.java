 ...
 * debug issues in the state machine implementation. It can also hide the step
...
public class SimpleGameSim
{
  ...
  {
    final Game theGame = GameRepository.getDefaultRepository()
        .getGame("nineBoardTicTacToe");
    ...
    try
    {
      ...
      theMatch
          .setCryptographicKeys(new EncodedKeyPair(FileUtils
              .readFileAsString(new File("src/org/ggp/base/apps/utilities/SampleKeys.json"))));
    }
    ...
    theServer.addObserver(new Observer()
    {
      ...
      public void observe(Event event)
      {
        ...
        {
          MachineState theCurrentState = ((ServerNewGameStateEvent)event)
              .getState();
          if (nState[0] > 0)
          ...
          for (GdlSentence newSentence : newContents)
          {
            if (hideStepCounter && newSentence.toString().contains("step"))
              continue;
            if (hideControlProposition &&
                newSentence.toString().contains("control"))
              continue;
          ...
          for (GdlSentence oldSentence : oldContents)
          {
            if (hideStepCounter && oldSentence.toString().contains("step"))
              continue;
            if (hideControlProposition &&
                oldSentence.toString().contains("control"))
              continue;
          ...
          if (showCurrentState)
            System.out.println("State[" + nState[0] + "] Full: " +
                               theCurrentState);
          nState[0]++;
        }
        else if (event instanceof ServerNewMovesEvent)
        {
          System.out.println("Move taken: " +
                             ((ServerNewMovesEvent)event).getMoves());
        }
        else if (event instanceof ServerCompletedMatchEvent)
        {
          System.out.println("State[" + nState[0] + "] Full (Terminal): " +
                             oldContents);
          System.out.println("Match information: " + theMatch);
          System.out.println("Goals: " +
                             ((ServerCompletedMatchEvent)event).getGoals());
          try
          {
            System.out.println("Match information cryptographically signed? " +
                               SignableJSON
                                   .isSignedJSON(new JSONObject(theMatch
                                       .toJSON())));
            System.out
                .println("Match information cryptographic signature valid? " +
                         SignableJSON.verifySignedJSON(new JSONObject(theMatch
                             .toJSON())));
          }
