                                                 ...
                                                 TableModelListener
{
  ...
  public SchedulingPanel()
  {
    ...
    add(new JScrollPane(queueTable,
          ...
          if (matchIdToURL.containsKey(matchId))
          {
            try
            {
              java.awt.Desktop.getDesktop()
                  .browse(java.net.URI.create(matchIdToURL.get(matchId)));
            }
      ...
      public void actionPerformed(ActionEvent evt)
      {
        if (queueTable.getSelectedRow() >= 0)
        {
          ...
          if (matchIdToFilename.containsKey(matchId))
          {
            ...
            {
              java.awt.Desktop.getDesktop()
                  .browse(java.net.URI.create("file://" +
                                              matchIdToFilename.get(matchId)));
            }
            catch (IOException ie)
            {
              ie.printStackTrace();
  ...
  public void addPendingMatch(PendingMatch spec)
  {
    ...
    model.addRow(new Object[] {spec.matchID, spec.theGame.getKey(),
        spec.startClock + "," + spec.playClock, "pending",
        getLinebreakString(getNamesForPlayers(spec.thePlayers)), "", "", 0});
    queueTable.setRowHeight(model.getRowCount() - 1,
  ...
  public void observe(Event genericEvent)
  {
    ...
    for (int i = 0; i < model.getRowCount(); i++)
    {
      ...
      if (rowMatchId.equals(match.getMatchId()))
      {
        ...
        {
          matchIdToURL.put(match.getMatchId(),
                           "http://www.ggp.org/view/all/matches/" +
                               event.getExternalPublicationKey() + "/");
        }
    ...
    model.addRow(new Object[] {match.getMatchId(), match.getGame().getKey(),
        match.getStartClock() + "," + match.getPlayClock(), "starting",
        getLinebreakString(match.getPlayerNamesFromHost()), "", "", 0});
    queueTable.setRowHeight(model.getRowCount() - 1, match
        .getPlayerNamesFromHost().size() * 20);
  }
  ...
  private static String getLinebreakString(List<?> objects)
  {
    ...
    return renderedString.substring(0,
                                    renderedString.length() - "<br>".length()) +
           "</html>";
  }
}