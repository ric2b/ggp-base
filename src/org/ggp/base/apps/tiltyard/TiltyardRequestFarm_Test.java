...
public class TiltyardRequestFarm_Test extends TestCase
{
  ...
  /*
   * TODO(schreib): Get all of these working at the same time. public void
   * testConnectionError() { new ReceiverLoopThread("CE").start();
   * runTestingLoop(); } public void testTimeout() { new
   * ResponderLoopThread(4000).start(); new ReceiverLoopThread("TO").start();
   * runTestingLoop(); }
   */
  ...
  class ReceiverThread extends Thread
  {
    ...
    public void run()
    {
      try
      {
        ...
        {
          long original = Long.parseLong(new JSONObject(responseJSON
              .getString("originalRequest")).getString("requestContent"));
      ...
      while (true)
      {
        ...
        assertEquals("okay",
                     RemoteResourceLoader
                         .postRawWithTimeout("http://127.0.0.1:" +
                                                 TiltyardRequestFarm.SERVER_PORT,
                                             theRequest.toString(),
                                             5000));
        nRequests++;
        Thread.sleep(10);
        synchronized (nSuccesses)
        {
          System.out.println("Successes so far: " + nSuccesses +
                             " vs Requests: " + nRequests);
          if (nSuccesses > 3000)
          {
