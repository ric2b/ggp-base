 ...
 * connections, makes requests, and reports back responses on behalf of a
 * remote client. It serves as a backend for intermediary systems that, due to
 * various restrictions, cannot make long-lived HTTP connections themselves.
 * This is the backend for the continuously-running online GGP.org Tiltyard,
 * which schedules matches between players around the world and aggregates
 * stats based on the outcome of those matches. SAMPLE INVOCATION (when running
 * locally): ResourceLoader.load_raw('http://127.0.0.1:9124/' +
 ...
 * "requestContent":"( play foo bar baz )"}))) Tiltyard Request Farm will open
 * up a network connection to the target, send the request string, and wait for
 * the response. Once the response arrives, it will close the connection and
 * call the callback, sending the response to the remote client that issued the
...
public final class TiltyardRequestFarm
{
  ...
  public static final EncodedKeyPair theBackendKeys = getKeyPair(FileUtils
                                                        .readFileAsString(new File("src/org/ggp/base/apps/tiltyard/BackendKeys.json")));
  ...
  public static String generateSignedPing()
  {
    ...
    try
    {
      ...
      SignableJSON.signJSON(thePing,
                            theBackendKeys.thePublicKey,
                            theBackendKeys.thePrivateKey);
    }
    catch (JSONException e)
    {
      e.printStackTrace();
  ...
  {
    String      targetHost, requestContent, forPlayerName, callbackURL,
        originalRequest;
    ...
    public void run()
    {
      if (originalRequest != null)
      {
        ...
        System.out.println("On " + new Date() +
                           ", starting request. There are now " +
        ...
        try
        {
          ...
          try
          {
            String response = HttpRequest.issueRequest(targetHost,
                                                       targetPort,
                                                       forPlayerName,
                                                       requestContent,
                                                       timeoutClock);
          ...
          if (!testMode)
          {
            SignableJSON.signJSON(responseJSON,
                                  theBackendKeys.thePublicKey,
                                  theBackendKeys.thePrivateKey);
          }
        }
        ...
        while (true)
        {
          try
          {
            RemoteResourceLoader.postRawWithTimeout(callbackURL,
        ...
        synchronized (ongoingRequests)
        {
          ...
          if (ongoingRequests == 0)
          {
            ...
            System.out
                .println("On " + new Date() +
                         ", completed request. Garbage collecting since there are no ongoing requests.");
          }
          else
          {
            System.out.println("On " + new Date() +
                               ", completed request. There are now " +
                               ongoingRequests + " ongoing requests.");
          }
        }
        synchronized (activeRequests)
  ...
  static class TiltyardRegistration extends Thread
  {
    ...
    public void run()
    {
      ...
      while (true)
      {
        try
        {
          RemoteResourceLoader.postRawWithTimeout(registrationURL,
                                                  generateSignedPing(),
                                                  2500);
        }
        catch (Exception e)
  ...
  public static void main(String[] args)
  {
    ...
    if (!testMode)
    {
      ...
      {
        System.err
            .println("Could not load cryptographic keys for signing request responses.");
        return;
      }
    ...
    while (true)
    {
      try
      {
        ...
        RunRequestThread handlerThread = new RunRequestThread(connection,
                                                              activeRequests);
        handlerThread.start();
      }
      catch (Exception e)
