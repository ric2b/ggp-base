...
public class CommonGraphics
{
  ...
  public static Image getImage(String dirName, String imageName)
  {
    ...
    {
      return ImageIO
          .read(new File(new File(ProjectConfiguration.gameImagesDirectory,
                                  dirName), imageName));
    }
    catch (IOException e)
    {
  ...
  public static void fillWithString(Graphics g,
                                    String theText,
                                    double sizeFactor)
  {
    ...
    Font theFont = g.getFont().deriveFont((float)(theHeight / sizeFactor))
        .deriveFont(Font.BOLD);
    g.setFont(theFont);

    FontMetrics theMetric = g.getFontMetrics();
    g.drawString(theText,
                 (theWidth - theMetric.stringWidth(theText)) / 2,
                 theMetric.getAscent() +
                     (theHeight - (theMetric.getDescent() + theMetric
                         .getAscent())) / 2);
  }
  ...
  public static void drawCheckersPiece(Graphics g, String checkersPiece)
  {
    ...
    Color theColor = ((checkersPiece.charAt(0) == 'b') ? Color.BLACK
                                                      : Color.RED);
  ...
  public static void drawChessPiece(Graphics g, String chessPiece)
  {
    ...
    {
      if (chessPiece.equals("wp"))
        toDraw = whitePawnImage;
      else if (chessPiece.equals("wn"))
        toDraw = whiteKnightImage;
      else if (chessPiece.equals("wb"))
        toDraw = whiteBishopImage;
      else if (chessPiece.equals("wq"))
        toDraw = whiteQueenImage;
      else if (chessPiece.equals("wr"))
        toDraw = whiteRookImage;
      else if (chessPiece.equals("wk"))
        toDraw = whiteKingImage;
    }
    else if (chessPiece.charAt(0) == 'b')
    {
      if (chessPiece.equals("bp"))
        toDraw = blackPawnImage;
      else if (chessPiece.equals("bn"))
        toDraw = blackKnightImage;
      else if (chessPiece.equals("bb"))
        toDraw = blackBishopImage;
      else if (chessPiece.equals("bq"))
        toDraw = blackQueenImage;
      else if (chessPiece.equals("br"))
        toDraw = blackRookImage;
      else if (chessPiece.equals("bk"))
        toDraw = blackKingImage;
    }
    ...
    {
      System.err
          .println("Could not process chess piece [" + chessPiece + "].");
    }
  }
