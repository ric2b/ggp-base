...
public class PentagoCanvas extends GameCanvas_FancyGrid
{
  ...
  protected Set<String> getFactsAboutCell(int xCell, int yCell)
  {
    ...
    return gameStateHasFactsMatching("\\( cellHolds " + nQuad + " " + xCell +
                                     " " + yCell + " (.*) \\)");
  }
  ...
  protected Set<String> getLegalMovesForCell(int xCell, int yCell)
  {
    ...
    xCell = ((xCell > 3) ? (xCell - 3) : xCell);
    yCell = ((yCell > 3) ? (yCell - 3) : yCell);
    Set<String> theMoves = gameStateHasLegalMovesMatching("\\( place " +
                                                          nQuad + " " + xCell +
                                                          " " + yCell + " \\)");
  ...
  protected void renderCellContent(Graphics g, String theFact)
  {
    ...
    CommonGraphics
        .drawCheckersPiece(g, cellPlayer.equals("red") ? "wp" : "bp");
  }

  private Image left_arrow  = CommonGraphics
                                .getImage("pentago", "BlueCircleArrowCCW.png");
  private Image right_arrow = CommonGraphics.getImage("pentago",
                                                      "BlueCircleArrowCW.png");
