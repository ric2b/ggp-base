...
public class TicTacToeCanvas_Reference extends GameCanvas
{
  ...
  protected void paintGame(Graphics g)
  {
    ...
    for (GdlSentence stateFact : gameState.getContents())
    {
      ...
      {
        int xCell = Integer.parseInt(theRealSentence.getBody().get(0)
            .toString());
        int yCell = Integer.parseInt(theRealSentence.getBody().get(1)
            .toString());
        ...
        else if (mark == 'x')
        {
          g.drawLine(xSpot + BORDER_SIZE,
                     ySpot + BORDER_SIZE,
                     xSpot + width / 3 - BORDER_SIZE,
                     ySpot + height / 3 - BORDER_SIZE);
          g.drawLine(xSpot + BORDER_SIZE,
                     ySpot + height / 3 - BORDER_SIZE,
                     xSpot + width / 3 - BORDER_SIZE,
                     ySpot + BORDER_SIZE);
        ...
        {
          g.drawOval(xSpot + BORDER_SIZE,
                     ySpot + BORDER_SIZE,
                     width / 3 - 2 * BORDER_SIZE,
                     height / 3 - 2 * BORDER_SIZE);
        }
      }
    }
    ...
    try
    {
      ...
      for (Move legalMove : legalMoves)
      {
        ...
        {
          g.drawRect(xSpot + BORDER_SIZE / 2,
                     ySpot + BORDER_SIZE / 2,
                     width / 3 - BORDER_SIZE,
                     height / 3 - BORDER_SIZE);
        }
      }
    }
