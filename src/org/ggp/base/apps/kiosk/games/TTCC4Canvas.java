...
public class TTCC4Canvas extends GameCanvas_FancyGrid
{
  ...
  protected Set<String> getLegalMovesForCell(int xCell, int yCell)
  {
    ...
    Set<String> theMoves = gameStateHasLegalMovesMatching("\\( pawnMove " +
                                                          xCell + " " + yCell +
                                                          " (.*) \\)");
    theMoves.addAll(gameStateHasLegalMovesMatching("\\( knightMove " + xCell +
                                                   " " + yCell + " (.*) \\)"));
    theMoves.addAll(gameStateHasLegalMovesMatching("\\( checkerMove " + xCell +
                                                   " " + yCell + " (.*) \\)"));
    theMoves.addAll(gameStateHasLegalMovesMatching("\\( jump " + xCell + " " +
                                                   yCell + " (.*) \\)"));
  ...
  protected void renderCellContent(Graphics g, Set<String> theFacts)
  {
    ...
    {
      Color myColor = null;
      if (cellType.startsWith("light"))
        myColor = Color.red;
      if (cellType.startsWith("dark"))
        myColor = Color.blue;
      if (myColor == null)
      {
