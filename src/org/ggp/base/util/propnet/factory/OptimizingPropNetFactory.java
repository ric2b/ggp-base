...
public class OptimizingPropNetFactory
{
  ...
  private static void removeUselessBasePropositions(Map<GdlSentence, Component> components,
                                                    Map<GdlSentence, Component> negations,
                                                    Constant trueComponent,
                                                    Constant falseComponent)
      throws InterruptedException
  {
  ...
  private static void processTemporaryComponents(Map<GdlSentence, Component> temporaryComponents,
      ...
      throws InterruptedException
  {
    while (hasNonessentialChildren(trueComponent) ||
           hasNonessentialChildren(falseComponent))
    {
      ConcurrencyUtils.checkForInterruption();
  ...
  private static void optimizeAwayTrueAndFalse(PropNet pn,
                                               Component trueComponent,
                                               Component falseComponent)
  {
    while (hasNonessentialChildren(trueComponent) ||
  ...
  private static void optimizeAwayFalse(Map<GdlSentence, Component> components,
                                        Map<GdlSentence, Component> negations,
                                        PropNet pn,
                                        Component trueComponent,
                                        Component falseComponent)
  ...
  private static void optimizeAwayTrue(Map<GdlSentence, Component> components,
                                       Map<GdlSentence, Component> negations,
                                       PropNet pn,
                                       Component trueComponent,
                                       Component falseComponent)
   ...
   */
  private static void orify(Collection<Component> inputs,
                            Component output,
                            Constant falseProp)
  {
  ...
  private static boolean isThisConstant(Component conj,
                                        Constant constantComponent)
  {
    if (conj == constantComponent)
      return true;
    return (conj instanceof Proposition && conj.getInputs().size() == 1 && conj
        .getSingleInput() == constantComponent);
  }
  ...
  private static void andify(List<Component> inputs,
                             Component output,
                             Constant trueProp)
  {
  ...
  public static void removeUnreachableBasesAndInputs(PropNet pn)
  {
    ...
    for (Entry<GdlSentence, Proposition> entry : pn.getBasePropositions()
        .entrySet())
    {
  ...
  public static void removeRedundantConstantsAndGates(PropNet pn)
  {
    ...
    do
    {
      ...
      for (Component c : pn.getComponents())
      {
        if (c instanceof Constant && c != trueConst && c != falseConst)
        {
        ...
        else
        {
          if (c instanceof And)
          {
            if (c.getOutputs().size() == 1 &&
                c.getSingleOutput() instanceof And)
            {
              redundantComponents.add(c);
            }
          }
          ...
          {
            if (c.getOutputs().size() == 1 &&
                c.getSingleOutput() instanceof Or)
            {
              redundantComponents.add(c);
            }
          }
          ...
          {
            if (c.getOutputs().size() == 1 &&
                c.getSingleOutput() instanceof Not)
            {
              redundantComponents.add(c);
            }
          }
  ...
  public static void removeAnonymousPropositions(PropNet pn)
  {
    ...
    for (Proposition p : pn.getPropositions())
    {
      ...
      if (p.getInputs().size() == 1 &&
          p.getSingleInput() instanceof Transition)
