 ...
 * the rules to contain ( base ?x ) propositions that explicitly specify
 * domains for all of the base propositions. This flattener should work on all
 * sizes of games, but requires these explicit annotations to be present in the
 * game description; it cannot infer them. A separate PropNetAnnotater class is
 * able to generate these annotations. Sadly, it can only annotate relatively
 * simple games. If there are no base propositions in a game description,
 * PropNetAnnotatedFlattener will call the annotater in an attempt to generate
 * annotations. To use this class: PropNetAnnotatedFlattener AF = new
 * PropNetAnnotatedFlattener(description); List<GdlRule> flatDescription =
 * AF.flatten(); return converter.convert(flatDescription);
 */
public final class PropNetAnnotatedFlattener
{
  ...
  public List<GdlRule> flatten()
  {
    ...
    if (noAnnotations())
    {
      GamerLogger
          .log("StateMachine",
               "Could not find 'base' annotations. Attempting to generate them...");
      description = new PropNetAnnotater(description)
          .getAugmentedDescription();
    ...
    List<GdlRule> flatDescription = new ArrayList<>();
   ...
   * <tt>(base name (arg1 a11 a12 ... a1n) ... (argn an1 an2 ... ann))</tt> the
   * method will return every possible combination of rules with name
   ...
   */
  private void expandTrue(GdlSentence base,
                          int index,
                          LinkedList<GdlTerm> workingSet,
                          List<GdlRule> results)
  {
    if (base.arity() == index)
    {
      GdlConstant name = (GdlConstant)base.get(0);
      List<GdlTerm> body = new ArrayList<>(workingSet);

      GdlFunction function = GdlPool.getFunction(name, body);
      results.add(GdlPool.getRule(GdlPool.getRelation(GdlPool
          .getConstant("true"), new GdlTerm[] {function})));
    }
    else
    {
   ...
   * <tt>constant</tt>, building one first if necessary. Rules with heads named
   * <tt>legal</tt> are generated by looking up the instantiation for rules
   * with heads named <tt>does</tt> and renaming the results.
   *
   * @param constant
  ...
  private List<GdlRule> getInstantiations(GdlConstant constant)
  {
    if (!instantiations.containsKey(constant))
    {
      ...
      if (constant.getValue().equals("does"))
      {
        ...
        {
          GdlSentence head = GdlPool.getRelation(GdlPool.getConstant("does"),
                                                 rule.getHead().getBody());
      ...
      else
      {
        ...
        {
          Set<GdlRule> results = new HashSet<>();
  ...
  {
    List<GdlRule> trues = new ArrayList<>();
    for (Gdl gdl : description)
    {
      if (gdl instanceof GdlSentence)
      {
        ...
        if (sentence.getName().getValue().equals("base"))
        {
          if (sentence.arity() == 1)
          {
            ...
            trues.add(GdlPool.getRule(GdlPool.getRelation(GdlPool
                .getConstant("true"), new GdlTerm[] {constant})));
          }
          else
          {
            List<GdlRule> results = new ArrayList<>();
    ...
    Map<GdlConstant, List<GdlRule>> instantiations = new HashMap<>();
   ...
   *          The list of results built up so far.
   */
  private void instantiate(GdlRule template,
                           int index,
                           Substitution theta,
                           Set<GdlRule> results)
  {
    ...
    else
    {
      ...
      {
        GdlSentence sentence = (GdlSentence)Substituter.substitute(literal,
                                                                   theta);
        for (GdlRule instantiation : getInstantiations(sentence.getName()))
        {
          Substitution thetaPrime = Unifier.unify(sentence,
                                                  instantiation.getHead());
          if (thetaPrime != null)
          {
  ...
  {
    Map<GdlConstant, List<GdlRule>> templates = new HashMap<>();
    for (Gdl gdl : description)
    {
      GdlRule rule = (gdl instanceof GdlRule) ? (GdlRule)gdl : GdlPool
          .getRule((GdlSentence)gdl);
