...
public class PropNetFlattener
{
  ...
  private class RuleReference
  {
    ...
    {
      return "\n\tProduction: " +
             (productionTemplate != null ? productionTemplate.toString()
                                        : "null") + " conditions: " +
    ...
    public boolean equals(Object other)
    {
      ...
      return rhs.productionTemplate == this.productionTemplate &&
             rhs.conditions.equals(this.conditions);
    }
  ...
  {
    public Domain(GdlTerm name, GdlTerm name2)
    {
      this.name = name;
      this.name2 = name2;
    }
  ...
  private List<GdlRule> getAllInstantiations()
  {
    ...
    for (RuleReference r : extraRefs)
    {
      ...
      if (varInstantiations.size() == 0)
        rval.add(getInstantiation(r.originalRule,
                                  new HashMap<GdlVariable, GdlConstant>()));
    }

    return rval;
  }

  private GdlRule getInstantiation(Gdl gdl,
                                   Map<GdlVariable, GdlConstant> varInstantiation)
  {
    Gdl instant = getInstantiationAux(gdl, varInstantiation);
    return (GdlRule)instant;
  }

  private Gdl getInstantiationAux(Gdl gdl,
                                  Map<GdlVariable, GdlConstant> varInstantiation)
  {
    if (gdl instanceof GdlRelation)
    {
    ...
    else if (gdl instanceof GdlDistinct)
    {
      ...
      GdlTerm arg1 = (GdlTerm)getInstantiationAux(distinct.getArg1(),
                                                  varInstantiation);
      GdlTerm arg2 = (GdlTerm)getInstantiationAux(distinct.getArg2(),
                                                  varInstantiation);
    ...
    else if (gdl instanceof GdlNot)
    {
      ...
      GdlLiteral body = (GdlLiteral)getInstantiationAux(not.getBody(),
                                                        varInstantiation);
  ...
  void updateDomains()
  {
    ...
    while (changedSomething)
    {
      ...
      GamerLogger.log("StateMachine", "\tDone with iteration.  Considered " +
  ...
  private Set<Map<GdlVariable, GdlConstant>> findSatisfyingInstantiations(List<Condition> conditions,
                                                                          int idx,
                                                                          Map<GdlVariable, GdlConstant> instantiation)
  {
      ...
      if (c != null)
      {
        ...
        else
        {
          if (dom.indices.size() > i)
          {
            ...
            //This is when we've tried to find an assignment for a form that doesn't have any assignments yet.  Pretend it returned an empty set
            assignments.clear();
        }
      }
    }
    ...
    for (Assignment a : assignments)
    {
      ...
      rval.addAll(findSatisfyingInstantiations(conditions,
                                               idx + 1,
                                               newInstantiation));
    }
