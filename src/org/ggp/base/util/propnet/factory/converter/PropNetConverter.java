...
public final class PropNetConverter
{
  ...
  /**
   * Converts a game description to a PropNet using the following process (note
   * that this method and all of the methods that it invokes assume that
   * <tt>description</tt> has already been flattened by a PropNetFlattener):
   * <ol>
   * <li>Transforms each of the rules in <tt>description</tt> into equivalent
   * PropNet Components.</li>
  ...
  /**
   * Creates an or gate to combine the inputs to a Proposition wherever one has
   * more than one input.
   */
    ...
    for (Proposition fixItem : fixList)
    {
      ...
      for (Component input : fixItem.getInputs())
      {
        ...
        if (fixItem.getName() instanceof GdlProposition)
        {
          ...
          disjunct = new Proposition(GdlPool.getProposition(GdlPool
              .getConstant(proposition.getName().getValue() + "-" + i)));
        }
        else
        {
          GdlRelation relation = (GdlRelation)fixItem.getName();
          disjunct = new Proposition(GdlPool.getRelation(GdlPool
              .getConstant(relation.getName().getValue() + "-" + i), relation
              .getBody()));
        }
