...
public class PolymorphicPropNet
{
	 ...
	 */
	public PolymorphicPropNet(List<Role> roles, Set<PolymorphicComponent> components, PolymorphicComponentFactory componentFactory)
	{
	...
	private Map<PolymorphicProposition, PolymorphicProposition> makeLegalInputMap() {
		...
		for(Set<PolymorphicProposition> legalProps : legalPropositionsMutable.values()) {
			for(PolymorphicProposition legalProp : legalProps) {
				...
				if (inputPropsByBody.containsKey(legalPropBody)) {
					PolymorphicProposition inputProp = inputPropsByBody.get(legalPropBody);
	...
	private Map<Role, Set<PolymorphicProposition>> recordGoalPropositions()
	{
		...
		for (PolymorphicProposition proposition : propositions)
		{
			...
			if (!goalPropositionsMutable.containsKey(theRole)) {
				goalPropositionsMutable.put(theRole, new HashSet<PolymorphicProposition>());
			}
	...
	public Map<Role, PolymorphicProposition[]> getLegalPropositions()
	{
		if ( legalPropositions == null )
		{
			...
			{
				PolymorphicProposition[] legalsForRole = new PolymorphicProposition[legalPropositionsMutable.get(role).size()];
				int index = 0;
	...
	public void removeComponent(PolymorphicComponent c) {
		...
		if(c instanceof PolymorphicProposition) {
			...
			if(basePropositions.containsKey(name)) {
				...
				for(Set<PolymorphicProposition> propositions : legalPropositionsMutable.values()) {
				...
				for(Set<PolymorphicProposition> propositions : goalPropositionsMutable.values()) {
					goalPropositions = null;
					propositions.remove(p);
				}
			}
