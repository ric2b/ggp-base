...
public class PieceHeuristicAnalyser extends Analyser
{
		...
		for(PotentialPiecePropSet pieceSet : potentialPiecePropSets)
		{
			...
			for(String roleArgValue : info.paramRanges.get(pieceSet.potentialRoleArgIndex))
			{
				...
				if ( pieceSetSentences.size() >= minPiecesThreshold )
				{
					...
					ForwardDeadReckonInternalMachineState pieceMask = stateMachine.createInternalState(new MachineState(pieceSetSentences));
	...
	@Override
	public void accrueInterimStateSample(ForwardDeadReckonInternalMachineState sampleState, int choosingRoleIndex)
	{
	...
	@Override
	public void accrueTerminalStateSample(ForwardDeadReckonInternalMachineState finalState, double[] roleScores)
	{
    	for(Entry<ForwardDeadReckonInternalMachineState, HeuristicScoreInfo> e : propGroupScoreSets.entrySet())
    	{
