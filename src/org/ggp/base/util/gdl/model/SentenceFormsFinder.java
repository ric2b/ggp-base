...
public class SentenceFormsFinder
{
  ...
  private final Map<NameAndArity, List<TermModel>> sentencesModel   = Maps
                                                                        .newHashMap();
  ...
  public ImmutableSet<SentenceForm> findSentenceForms()
  ...
  public Map<SentenceForm, SentenceFormDomain> findCartesianDomains()
      throws InterruptedException
  {
    createModel();
  ...
  private Map<SentenceForm, SentenceFormDomain> getCartesianDomainsFromModel()
      throws InterruptedException
  {
    Map<SentenceForm, SentenceFormDomain> results = Maps.newHashMap();
    for (Entry<NameAndArity, List<TermModel>> sentenceEntry : sentencesModel
        .entrySet())
    {
      ...
      if (nameAndArity.getArity() == 0)
      {
        ...
        results.put(form, CartesianSentenceFormDomain
            .create(form, ImmutableList.<Set<GdlConstant>> of()));
      }
      else
      {
  ...
  private SentenceFormDomain getDomain(SentenceForm form, GdlRelation sentence)
  {
    ...
    getDomainInternal(sentence.getBody(),
                      sentencesModel.get(new NameAndArity(sentence)),
                      domainContents);
    return CartesianSentenceFormDomain.create(form, domainContents);
  }

  //Appends to domainContents
  private void getDomainInternal(List<GdlTerm> body,
                                 ...
                                 List<Set<GdlConstant>> domainContents)
  {
    ...
    for (int i = 0; i < body.size(); i++)
    {
      ...
      else if (term instanceof GdlFunction)
      {
        ...
        List<TermModel> functionBodyModel = termModel
            .getFunctionBodyModel(function);
        getDomainInternal(function.getBody(),
                          functionBodyModel,
                          domainContents);
      }
      else
  ...
  private Set<GdlTerm> toSampleTerms(TermModel termModel)
  {
    ...
    {
      List<TermModel> bodyModel = termModel.getPossibleFunctions()
          .get(nameAndArity);
      List<Set<GdlTerm>> functionSampleTerms = toSampleTerms(bodyModel);
      Set<List<GdlTerm>> functionBodies = Sets
          .cartesianProduct(functionSampleTerms);
      for (List<GdlTerm> functionBody : functionBodies)
      {
        GdlFunction function = GdlPool.getFunction(nameAndArity.getName(),
                                                   functionBody);
  ...
  private boolean applyLanguageRules() throws InterruptedException
  {
    ...
    changesMade |= applyInjection(new NameAndArity(GdlPool.INIT, 1),
                                  new NameAndArity(GdlPool.TRUE, 1));
    changesMade |= applyInjection(new NameAndArity(GdlPool.NEXT, 1),
                                  new NameAndArity(GdlPool.TRUE, 1));
    changesMade |= applyInjection(new NameAndArity(GdlPool.LEGAL, 2),
                                  new NameAndArity(GdlPool.DOES, 2));
    return changesMade;
  }

  private boolean applyInjection(NameAndArity oldName, NameAndArity newName)
      throws InterruptedException
  {
    ConcurrencyUtils.checkForInterruption();
  ...
  {
    Set<GdlVariable> varsToUse = Sets.newHashSet(GdlUtils.getVariables(rule
        .getHead()));
    ...
    for (GdlLiteral literal : rule.getBody())
    {
      if (literal instanceof GdlRelation)
      {
        ...
        {
          sentencesModel.put(nameAndArity,
                             getNTermModels(nameAndArity.getArity()));
        }
                                 ...
                                 Map<GdlVariable, TermModel> varsToModelsMap)
  {
    ...
    {
      throw new IllegalArgumentException("The term model and body sizes don't match: model is " +
                                         model + ", body is: " + body);
    }
    for (int i = 0; i < body.size(); i++)
    {
      ...
      else if (term instanceof GdlFunction)
      {
        ...
        {
          addVariablesToMap(function.getBody(),
                            functionBodyModel,
                            varsToModelsMap);
        }
      }
  ...
  private void addTrueSentencesToModel() throws InterruptedException
  {
    for (Gdl gdl : description)
    {
      ...
      {
        addSentenceToModel((GdlSentence)gdl,
                           ImmutableMap.<GdlVariable, TermModel> of());
      }
    }
  }

  private boolean addSentenceToModel(GdlSentence sentence,
                                     Map<GdlVariable, TermModel> varsToModelsMap)
      throws InterruptedException
  {
    ConcurrencyUtils.checkForInterruption();
    ...
    changesMade |= addBodyToModel(sentencesModel.get(sentenceName),
                                  sentence.getBody(),
                                  varsToModelsMap);
    return changesMade;
  }
  ...
  private static boolean addBodyToModel(List<TermModel> model,
                                        List<GdlTerm> body,
                                        Map<GdlVariable, TermModel> varsToModelsMap)
  {
    boolean changesMade = false;
  ...
  {
    private final Set<GdlConstant>                   possibleConstants = Sets
                                                                           .newHashSet();
    private final Map<NameAndArity, List<TermModel>> possibleFunctions = Maps
                                                                           .newHashMap();
    ...
    public boolean mergeIn(TermModel other)
    {
      ...
      for (NameAndArity key : other.possibleFunctions.keySet())
      {
        ...
        else
        {
          ...
          {
            changesMade |= ourFunctionBodies.get(i)
    ...
    public boolean addTerm(GdlTerm term,
                           Map<GdlVariable, TermModel> varsToModelsMap)
    {
      boolean changesMade = false;
      if (term instanceof GdlConstant)
      ...
      else if (term instanceof GdlFunction)
      {
        ...
        if (!possibleFunctions.containsKey(sentenceName))
        {
          changesMade = true;
          possibleFunctions
              .put(sentenceName, getNTermModels(function.arity()));
        }
        changesMade |= addBodyToModel(possibleFunctions.get(sentenceName),
                                      function.getBody(),
                                      varsToModelsMap);
      }
      ...
      {
        throw new RuntimeException("Unrecognized term type " +
                                   term.getClass() + " for term " + term);
      }
      return changesMade;
    }
