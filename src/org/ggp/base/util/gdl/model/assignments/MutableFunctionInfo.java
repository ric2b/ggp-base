...
/**
 * Defines a {@link FunctionInfo} that can have values added to it (but not
 * removed from it) over time. This allows the functional info to stay correct
 * as new values are added with minimal additional computation. Not
 * thread-safe.
 */
public class MutableFunctionInfo implements AddibleFunctionInfo
{
  ...
  private MutableFunctionInfo(SentenceForm form)
  {
    ...
    for (int i = 0; i < form.getTupleSize(); i++)
    {
      ...
      valueMaps.add(Maps
          .<ImmutableList<GdlConstant>, GdlConstant> newHashMap());
    }
  }
  ...
  public static MutableFunctionInfo create(SentenceForm form,
                                           Collection<GdlSentence> initialSentences)
  {
  ...
  public void addTuple(List<GdlConstant> sentenceTuple)
  {
    ...
    for (int i = 0; i < sentenceTuple.size(); i++)
    {
      if (dependentSlots.get(i))
      {
        ...
        Map<ImmutableList<GdlConstant>, GdlConstant> valueMap = valueMaps
            .get(i);
        ...
        else
        {
          ...
          if (curValue != newValue)
          {
            ...
            valueMaps.set(i, ImmutableMap
                .<ImmutableList<GdlConstant>, GdlConstant> of());
          }
        }
      }
    }
