...
public class IterationOrderCandidate implements
                                    Comparable<IterationOrderCandidate>
{
  ...
  /**
   * This constructor is for "completing" the ordering by adding all remaining
   * variables, in some arbitrary order. No source conjuncts or functions are
   * added.
   */
  ...
  public IterationOrderCandidate(IterationOrderCandidate parent, int i)
  {
    ...
    List<GdlVariable> varsFromConjunct = GdlUtils
        .getVariables(sourceConjunctCandidate);
  ...
  public IterationOrderCandidate(IterationOrderCandidate parent,
                                 ...
                                 int functionalSentenceIndex,
                                 GdlVariable functionOutput)
  {
    //Shared rules:
    this.varsToAssign = parent.varsToAssign;
    this.sourceConjunctCandidates = parent.sourceConjunctCandidates;
  ...
  private List<IterationOrderCandidate> getSourceConjunctChildren()
  {
    ...
    if (!sourceConjunctIndices.isEmpty())
      lastSourceConjunctIndex = sourceConjunctIndices
          .get(sourceConjunctIndices.size() - 1);

    for (int i = lastSourceConjunctIndex + 1; i < sourceConjunctCandidates
        .size(); i++)
    {
    ...
    if (analyticFunctionOrdering && functionalSentencesInfo.size() > 8)
    {
      ...
      for (int i = 0; i < functionalSentencesInfo.size(); i++)
      {
        ...
        for (GdlVariable producibleVar : producibleVars)
        {
          ...
          if (!functionsHavingDependencies.containsKey(dependencies))
            functionsHavingDependencies.put(dependencies,
                                            new HashSet<Integer>());
      ...
      while (varOrdering.size() < varsToAssign.size())
      {
        ...
        else
        {
          ...
          for (Set<GdlVariable> dependencySet : functionsHavingDependencies
              .keySet())
          {
        ...
        Set<Integer> functions = functionsHavingDependencies
            .get(dependencySetToUse);
        int functionToUse = -1;
              ...
              .entrySet())
          {
            ...
            if (!newFunctionsHavingDependencies.containsKey(newKey))
              newFunctionsHavingDependencies.put(newKey,
                                                 new HashSet<Integer>());
            newFunctionsHavingDependencies.get(newKey)
  ...
  {
    return varOrdering.toString() + " with sources " +
           getSourceConjuncts().toString() + "; functional?: " +
           functionalConjunctIndices + "; domain sizes are " +
           this.varDomainSizes;
  }
}