...
public class AssignmentIteratorImpl implements AssignmentIterator
{
  ...
  private void incrementIndex(int index)
  {
    ...
    if (plan.getValuesToCompute() != null &&
        plan.getValuesToCompute().containsKey(index))
    {
  ...
  private void updateNextAssignment()
  {
    ...
    {
      ImmutableList<ImmutableList<GdlConstant>> tuples = plan
          .getTuplesBySource().get(s);
    ...
    for (int i = 0; i < valueIndices.size(); i++)
    {
      if ((plan.getValuesToCompute() == null || !plan.getValuesToCompute()
          .containsKey(i)) && plan.getSourceDefiningSlot().get(i) == -1)
      {
        nextAssignment.set(i,
                           plan.getValuesToIterate().get(i)
