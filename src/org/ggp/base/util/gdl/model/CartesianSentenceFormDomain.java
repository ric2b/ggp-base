...
/**
 * A {@link SentenceFormDomain} implementation that stores which constant
 * values are possible for each slot of a sentence form. This is a more compact
 * representation than a {@link FullSentenceFormDomain}, but has less
 * expressive power.
 */
public class CartesianSentenceFormDomain implements SentenceFormDomain
{
  ...
  {
    return new CartesianSentenceFormDomain(form, ImmutableList.copyOf(Lists
        .transform(domainsForSlots,
  ...
  {
    return Iterators.transform(Sets.cartesianProduct(domainsForSlots)
        .iterator(), new Function<List<GdlConstant>, GdlSentence>()
    {
      @Override
