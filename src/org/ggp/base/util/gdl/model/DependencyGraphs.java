 ...
 * offers a variety of functionality for dealing with dependency graphs
 * expressed in the form of SetMultimaps. These multimaps are paired with sets
 * of all nodes, to account for the possibility of nodes not included in the
 * multimap representation. All methods assume that keys in multimaps depend on
 * their associated values, or in other words are downstream of or are children
 * of those values.
 */
public class DependencyGraphs
{
  ...
  /**
   * Returns all elements of the dependency graph that match the given
   * predicate, and any elements upstream of those matching elements. The graph
   * may contain cycles. Each key in the dependency graph depends on/is
   * downstream of its associated values.
   */
  public static <T> ImmutableSet<T> getMatchingAndUpstream(Set<T> allNodes,
  ...
  /**
   * Returns all elements of the dependency graph that match the given
   * predicate, and any elements downstream of those matching elements. The
   * graph may contain cycles. Each key in the dependency graph depends on/is
   * downstream of its associated values.
   */
  public static <T> ImmutableSet<T> getMatchingAndDownstream(Set<T> allNodes,
                                                             SetMultimap<T, T> dependencyGraph,
                                                             Predicate<T> matcher)
  {
    return getMatchingAndUpstream(allNodes,
  ...
  /**
   * Given a dependency graph, return a topologically sorted ordering of its
   * components, stratified in a way that allows for recursion and cycles.
   * (Each set in the list is one unordered "stratum" of elements. Elements may
   * depend on elements in earlier strata or the same stratum, but not in later
   * strata.) If there are no cycles, the result will be a list of singleton
   * sets, topologically sorted. Each key in the given dependency graph depends
   * on/is downstream of its associated values.
   */
  public static <T> List<Set<T>> toposortSafe(Set<T> allElements,
                                              Multimap<T, T> dependencyGraph)
  {
    ...
    while (!strataToAdd.isEmpty())
    {
      ...
      addOrMergeStratumAndAncestors(curStratum,
                                    ordering,
                                    strataToAdd,
                                                        ...
                                                        List<Set<T>> downstreamStrata)
  {
    if (downstreamStrata.contains(curStratum))
    {
      ...
      List<Set<T>> toMerge = downstreamStrata.subList(mergeStartIndex,
                                                      downstreamStrata.size());
      mergeStrata(Sets.newHashSet(toMerge), toAdd, strataDependencyGraph);
      return;
    }
    downstreamStrata.add(curStratum);
    for (Set<T> parent : ImmutableList.copyOf(strataDependencyGraph
        .get(curStratum)))
    {
      ...
      {
        addOrMergeStratumAndAncestors(parent,
                                      ordering,
                                      toAdd,
                                      strataDependencyGraph,
                                      downstreamStrata);
                                      ...
                                      SetMultimap<Set<T>, Set<T>> strataDependencyGraph)
  {
    ...
    //Change the values
    for (Entry<Set<T>, Set<T>> entry : ImmutableList
        .copyOf(strataDependencyGraph.entries()))
    {
  ...
  private static <T> SetMultimap<Set<T>, Set<T>> createStrataDependencyGraph(Multimap<T, T> dependencyGraph)
  {
    SetMultimap<Set<T>, Set<T>> strataDependencyGraph = HashMultimap.create();
    for (Entry<T, T> entry : dependencyGraph.entries())
    {
      strataDependencyGraph.put(ImmutableSet.of(entry.getKey()),
