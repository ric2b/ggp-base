...
public class GameFlow
{
  ...
  private void solveTurns(SentenceDomainModel model)
      throws InterruptedException
  {
    //Before we can do anything else, we need a topological ordering on our forms
    List<SentenceForm> ordering = getTopologicalOrdering(model.getIndependentSentenceForms(),
                                                         model
                                                             .getDependencyGraph());
    ...
    {
      functionInfoMap
          .put(form, FunctionInfoImpl.create(form, constantChecker));
    }
                                ...
                                Map<SentenceForm, FunctionInfo> functionInfoMap)
  {
    for (SentenceForm curForm : ordering)
    {
      ...
      for (GdlSentence alwaysTrueSentences : model
          .getSentencesListedAsTrue(curForm))
      ...
      for (GdlRule rule : model.getRules(curForm))
      {
        ...
        while (asnItr.hasNext())
        {
          ...
          GdlSentence transformedHead = CommonTransforms
  ...
  public Set<Integer> getTurnsConjunctsArePossible(List<GdlLiteral> body)
  {
    ...
    for (GdlLiteral literal : body)
    {
      if (literal instanceof GdlSentence)
      {
        ...
        if (SentenceModelUtils.inSentenceFormGroup(sentence,
                                                   formsControlledByFlow))
      ...
      else if (literal instanceof GdlNot)
      {
        ...
        if (SentenceModelUtils.inSentenceFormGroup(innerSentence,
                                                   formsControlledByFlow))
          relevantLiterals.add(literal);
      }
    }
      ...
      if (literal instanceof GdlSentence)
      {
        for (int t = 0; t < getNumTurns(); t++)
        {
          if (sentencesTrueByTurn.get(t).contains(literal))
            ...
            for (GdlSentence s : sentencesTrueByTurn.get(t))
            {
              //Could be true if there's an assignment
              if (null != GdlUtils
                  .getAssignmentMakingLeftIntoRight((GdlSentence)literal, s))
              {
                turns.add(t);
                break;
              }
      ...
      else if (literal instanceof GdlNot)
      {
        ...
        for (int t = 0; t < getNumTurns(); t++)
        {
          if (!sentencesTrueByTurn.get(t).contains(internal))
            ...
            for (GdlSentence s : sentencesTrueByTurn.get(t))
            {
              if (null != GdlUtils.getAssignmentMakingLeftIntoRight(internal,
                                                                    s))
              {
                turns.add(t);
                break;
