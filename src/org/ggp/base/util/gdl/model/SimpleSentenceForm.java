...
public class SimpleSentenceForm extends AbstractSentenceForm
{
  ...
  {
    return new SimpleSentenceForm(newName, arity, functions, tupleSize);
  }
  ...
  public GdlSentence getSentenceFromTuple(List<? extends GdlTerm> tuple)
  {
    if (tuple.size() != tupleSize)
    {
      throw new IllegalArgumentException("Passed tuple of the wrong size to a sentence form: " +
                                         "tuple was " +
                                         tuple +
                                         ", sentence form is " + this);
    }
    if (tuple.size() < arity)
    {
      throw new IllegalStateException("Something is very wrong, probably fixable by the GdlCleaner; name: " +
                                      name +
                                      "; arity: " +
                                      arity +
                                      "; tupleSize: " + tupleSize);
