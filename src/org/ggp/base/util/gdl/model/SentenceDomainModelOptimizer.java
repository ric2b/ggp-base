...
{
  /**
   * Given a SentenceDomainModel, returns an ImmutableSentenceDomainModel with
   * Cartesian domains that tries to minimize the domains of sentence forms
   * without impacting the game rules. In particular, when sentences are
   * restricted to these domains, the answers to queries about terminal, legal,
   * goal, next, and init sentences will not change. Note that if a sentence
   * form is not used in a meaningful way by the game, it may end up with an
   * empty domain. The description for the game must have had the
   * {@link VariableConstrainer} applied to it.
   */
  public static ImmutableSentenceDomainModel restrictDomainsToUsefulValues(SentenceDomainModel oldModel)
      throws InterruptedException
  {
    // Start with everything from the current domain model.
    Map<SentenceForm, SetMultimap<Integer, GdlConstant>> neededAndPossibleConstantsByForm = Maps
        .newHashMap();
    for (SentenceForm form : oldModel.getSentenceForms())
    {
      neededAndPossibleConstantsByForm.put(form, HashMultimap
          .<Integer, GdlConstant> create());
      addDomain(neededAndPossibleConstantsByForm.get(form),
                oldModel.getDomain(form),
                form);
    }

    /*
     * To minimize the contents of the domains, we repeatedly go through two
     * processes to reduce the domain: 1) We remove unneeded constants from the
     * domain. These are constants which (in their position) do not contribute
     * to any sentences with a GDL keyword as its name; that is, it never
     * matters whether a sentence with that constant in that position is true
     * or false. 2) We remove impossible constants from the domain. These are
     * constants which cannot end up in their position via any rule or sentence
     * in the game description, given the current domain. Constants removed
     * because of one type of pass or the other may cause other constants in
     * other sentence forms to become unneeded or impossible, so we make
     * multiple passes until everything is stable.
     */
    boolean somethingChanged = true;
    while (somethingChanged)
    {
      somethingChanged = removeUnneededConstants(neededAndPossibleConstantsByForm,
                                                 oldModel);
      somethingChanged |= removeImpossibleConstants(neededAndPossibleConstantsByForm,
                                                    oldModel);
    }
  ...
  private static void addDomain(SetMultimap<Integer, GdlConstant> setMultimap,
  ...
  private static boolean removeImpossibleConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                   SentenceFormModel model)
      throws InterruptedException
  {
    Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newPossibleConstantsByForm = Maps
        .newHashMap();
    for (SentenceForm form : curDomains.keySet())
    {
      newPossibleConstantsByForm.put(form, HashMultimap
          .<Integer, GdlConstant> create());
    }
    populateInitialPossibleConstants(newPossibleConstantsByForm,
                                     curDomains,
                                     model);
    ...
    while (somethingChanged)
    {
      somethingChanged = propagatePossibleConstants(newPossibleConstantsByForm,
                                                    curDomains,
                                                    model);
    }
  ...
  private static void populateInitialPossibleConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newPossibleConstantsByForm,
      ...
      throws InterruptedException
  {
    ...
    for (GdlRule rule : getRules(model.getDescription()))
    {
      ...
      addConstantsFromSentenceIfInOldDomain(newPossibleConstantsByForm,
                                            curDomains,
                                            model,
                                            head);
    }
  ...
  private static boolean propagatePossibleConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newPossibleConstantsByForm,
      ...
      throws InterruptedException
  {
    ...
    for (GdlRule rule : getRules(model.getDescription()))
    {
      ...
      for (GdlVariable varInHead : ImmutableSet.copyOf(GdlUtils
          .getVariables(rule.getHead())))
      {
        Set<GdlConstant> domain = getVarDomainInRuleBody(varInHead,
                                                         rule,
                                                         newPossibleConstantsByForm,
                                                         curDomain,
                                                         model);
        domainsOfHeadVars.put(varInHead, domain);
        somethingChanged |= addPossibleValuesToSentence(domain,
                                                        head,
                                                        varInHead,
                                                        newPossibleConstantsByForm,
                                                        model);
    ...
    somethingChanged |= applyLanguageBasedInjections(GdlPool.INIT,
                                                     GdlPool.TRUE,
                                                     newPossibleConstantsByForm);
    somethingChanged |= applyLanguageBasedInjections(GdlPool.NEXT,
                                                     GdlPool.TRUE,
                                                     newPossibleConstantsByForm);
    somethingChanged |= applyLanguageBasedInjections(GdlPool.LEGAL,
                                                     GdlPool.DOES,
                                                     newPossibleConstantsByForm);
  ...
  private static boolean applyLanguageBasedInjections(GdlConstant curName,
      ...
      throws InterruptedException
  {
    ...
    for (SentenceForm form : newPossibleConstantsByForm.keySet())
    {
      ...
      if (form.getName() == curName)
      {
        ...
        SetMultimap<Integer, GdlConstant> curFormDomain = newPossibleConstantsByForm
            .get(form);
        SetMultimap<Integer, GdlConstant> resultingFormDomain = newPossibleConstantsByForm
            .get(resultingForm);
  ...
  private static Set<GdlConstant> getVarDomainInRuleBody(GdlVariable varInHead,
                                                         GdlRule rule,
                                                         Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newPossibleConstantsByForm,
                                                         Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomain,
                                                         SentenceFormModel model)
  {
    try
    {
      ...
      for (GdlSentence conjunct : getPositiveConjuncts(rule.getBody()))
      {
        ...
        {
          domains.add(getVarDomainInSentence(varInHead,
                                             conjunct,
                                             newPossibleConstantsByForm,
                                             curDomain,
                                             model));
    ...
    {
      throw new RuntimeException("Error in rule " + rule + " for variable " +
                                 varInHead, e);
    }
  }

  private static Set<GdlConstant> getVarDomainInSentence(GdlVariable var,
  ...
  private static Set<GdlConstant> getIntersection(List<Set<GdlConstant>> domains)
  {
    if (domains.isEmpty())
    {
  ...
  private static boolean removeUnneededConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                 SentenceFormModel model)
      throws InterruptedException
  {
    Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newNeededConstantsByForm = Maps
        .newHashMap();
    for (SentenceForm form : curDomains.keySet())
    {
      newNeededConstantsByForm.put(form, HashMultimap
          .<Integer, GdlConstant> create());
    }
    ...
    while (somethingChanged)
    {
      somethingChanged = propagateNeededConstants(newNeededConstantsByForm,
                                                  curDomains,
                                                  model);
    }
  ...
  private static boolean retainNewDomains(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                          Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newDomains)
  {
    ...
    for (SentenceForm form : curDomains.keySet())
    {
      ...
      somethingChanged |= curDomains.get(form).entries()
          .retainAll(newDomain.entries());
    }
    return somethingChanged;
  }

  private static boolean propagateNeededConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> neededConstantsByForm,
                                                  Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                  SentenceFormModel model)
      throws InterruptedException
  {
    ...
    somethingChanged |= applyRuleHeadPropagation(neededConstantsByForm,
                                                 curDomains,
                                                 model);
    somethingChanged |= applyRuleBodyOnlyPropagation(neededConstantsByForm,
                                                     curDomains,
                                                     model);
  ...
  private static boolean applyRuleBodyOnlyPropagation(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> neededConstantsByForm,
                                                      Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                      SentenceFormModel model)
      throws InterruptedException
  {
    ...
    {
      GdlSentence head = rule.getHead();
      Set<GdlVariable> varsInHead = ImmutableSet.copyOf(GdlUtils
          .getVariables(head));

      Map<GdlVariable, Set<GdlConstant>> varDomains = getVarDomains(rule,
                                                                    curDomains,
                                                                    model);
      for (GdlVariable var : ImmutableSet.copyOf(GdlUtils.getVariables(rule)))
      {
        if (!varsInHead.contains(var))
        {
          ...
          {
            somethingChanged |= addPossibleValuesToConjunct(neededConstants,
                                                            conjunct,
                                                            var,
                                                            neededConstantsByForm,
                                                            model);
          }
        }
      }
  ...
  private static Map<GdlVariable, Set<GdlConstant>> getVarDomains(GdlRule rule,
                                                                  final Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                                  final SentenceFormModel model)
  {
    return SentenceDomainModels
        .getVarDomains(rule, new AbstractSentenceDomainModel(model)
        {
          @Override
  ...
  private static boolean applyRuleHeadPropagation(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> neededConstantsByForm,
                                                  Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                  SentenceFormModel model)
      throws InterruptedException
  {
    ...
    for (GdlRule rule : getRules(model.getDescription()))
    {
      ...
      for (int i = 0; i < headTuple.size(); i++)
      {
        ...
        if (headTuple.get(i) instanceof GdlVariable)
        {
          ...
          Set<GdlConstant> neededConstants = neededConstantsByForm
              .get(headForm).get(i);

          //Whittle these down based on what's possible throughout the rule
          Set<GdlConstant> neededAndPossibleConstants = Sets
              .newHashSet(neededConstants);
          neededAndPossibleConstants.retainAll(varDomains.get(curVar));
          //Relay those values back to the conjuncts in the rule body
          for (GdlLiteral conjunct : rule.getBody())
          {
            somethingChanged |= addPossibleValuesToConjunct(neededAndPossibleConstants,
                                                            conjunct,
                                                            curVar,
                                                            neededConstantsByForm,
                                                            model);
  ...
  private static boolean addPossibleValuesToConjunct(Set<GdlConstant> neededAndPossibleConstants,
                                                     GdlLiteral conjunct,
                                                     GdlVariable curVar,
      ...
      throws InterruptedException
  {
    ...
    {
      return addPossibleValuesToSentence(neededAndPossibleConstants,
                                         (GdlSentence)conjunct,
                                         curVar,
                                         neededConstantsByForm,
                                         model);
    ...
    else
    {
      throw new IllegalArgumentException("Unexpected literal type " +
                                         conjunct.getClass() +
                                         " for literal " + conjunct);
    }
  }

  private static boolean addPossibleValuesToSentence(Set<GdlConstant> neededAndPossibleConstants,
      ...
      throws InterruptedException
  {
    ...
    for (int i = 0; i < tuple.size(); i++)
    {
      if (tuple.get(i) == curVar)
      {
        ...
        somethingChanged |= neededConstantsByForm.get(form)
            .putAll(i, neededAndPossibleConstants);
      }
    }
    return somethingChanged;
  }
  ...
  {
    return Iterables.transform(Iterables.filter(body,
                                                new Predicate<GdlLiteral>()
                                                {
                                                  @Override
  ...
  {
    return Iterables.transform(Iterables.filter(description,
                                                new Predicate<Gdl>()
                                                {
                                                  @Override
  ...
  private static final ImmutableSet<GdlConstant> ALWAYS_NEEDED_SENTENCE_NAMES = ImmutableSet
                                                                                  .of(GdlPool.NEXT,
                                                                                      GdlPool.GOAL,
                                                                                      GdlPool.LEGAL,
                                                                                      GdlPool.INIT,
  ...
  private static void populateInitialNeededConstants(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newNeededConstantsByForm,
                                                     Map<SentenceForm, SetMultimap<Integer, GdlConstant>> curDomains,
                                                     SentenceFormModel model)
      throws InterruptedException
  {
    ...
    for (GdlRule rule : getRules(model.getDescription()))
    {
      ...
      {
        addConstantsFromSentenceIfInOldDomain(newNeededConstantsByForm,
                                              curDomains,
                                              model,
                                              sentence);
      }
  ...
  private static void addConstantsFromSentenceIfInOldDomain(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> newConstantsByForm,
  ...
  private static ImmutableSentenceDomainModel toSentenceDomainModel(Map<SentenceForm, SetMultimap<Integer, GdlConstant>> neededAndPossibleConstantsByForm,
                                                                    SentenceFormModel formModel)
      throws InterruptedException
  {
    ...
    for (SentenceForm form : formModel.getSentenceForms())
    {
      ...
      domains.put(form, CartesianSentenceFormDomain.create(form,
                                                           neededAndPossibleConstantsByForm
                                                               .get(form)));
    }
