...
public final class GdlPool
{
  ...
  public static final ImmutableSet<String>                                                   KEYWORDS        = ImmutableSet
                                                                                                                 .of("init",
                                                                                                                     "true",
                                                                                                                     "next",
                                                                                                                     "role",
  ...
  /**
   * Represents a single underscore ("_"). The underscore is not a GDL keyword,
   * but it's used by SentenceForms and is generally convenient for utility
   * methods.
   */
  public static final GdlConstant                                                            UNDERSCORE      = getConstant("_");

  /**
   * Drains the contents of the GdlPool. Useful to control memory usage once
   * you have finished playing a large game. WARNING: Should only be called
   * *between games*.
   */
  public static void drainPool()
  {
    ...
    constantPool.clear();
    constantCases.clear();
    for (Map.Entry<String, GdlConstant> keywordEntry : keywordConstants
        .entrySet())
    {
   ...
   * key to value to the pool. Note that even if you've checked to make sure
  ...
  public static GdlDistinct getDistinct(GdlTerm arg1, GdlTerm arg2)
  {
    ...
    if (bucket == null)
      bucket = addToPool(arg1,
                         new ConcurrentHashMap<GdlTerm, GdlDistinct>(),
                         distinctPool);
  ...
  public static GdlFunction getFunction(GdlConstant name, List<GdlTerm> body)
  {
    ...
    if (bucket == null)
    {
      ConcurrentMap<List<GdlTerm>, GdlFunction> newMap = new MapMaker()
          .softValues().makeMap();
  ...
  public static GdlRelation getRelation(GdlConstant name, List<GdlTerm> body)
  {
    ...
    if (bucket == null)
    {
      ConcurrentMap<List<GdlTerm>, GdlRelation> newMap = new MapMaker()
          .softValues().makeMap();
  ...
  public static GdlRule getRule(GdlSentence head, List<GdlLiteral> body)
  {
    ...
    if (bucket == null)
      bucket = addToPool(head,
                         new ConcurrentHashMap<List<GdlLiteral>, GdlRule>(),
                         rulePool);
  ...
  /**
   * This method should only rarely be used. It takes a foreign GDL object (one
   * that wasn't constructed through the GdlPool) and returns a version that
   * lives in the GdlPool. Various parts of the prover infrastructure expect
   * that all GDL objects live in the GdlPool, and so it's important that any
   * foreign GDL objects created outside the GdlPool be immersed before being
   * used. Since every GDL object should be created through the GdlPool,
   * immerse should only need to be called on GDL that appears from outside
   * sources: for example, being deserialized from a file.
   */
  public static Gdl immerse(Gdl foreignGdl)
  {
    if (foreignGdl instanceof GdlDistinct)
    {
      return GdlPool.getDistinct((GdlTerm)immerse(((GdlDistinct)foreignGdl)
          .getArg1()), (GdlTerm)immerse(((GdlDistinct)foreignGdl).getArg2()));
    }
    else if (foreignGdl instanceof GdlNot)
    {
      return GdlPool
          .getNot((GdlLiteral)immerse(((GdlNot)foreignGdl).getBody()));
    }
    ...
    {
      return GdlPool
          .getProposition((GdlConstant)immerse(((GdlProposition)foreignGdl)
              .getName()));
    }
