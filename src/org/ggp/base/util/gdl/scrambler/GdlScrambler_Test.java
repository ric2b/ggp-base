...
public class GdlScrambler_Test extends TestCase
{
   ...
   */
  public void testMappingScrambler()
      throws GdlFormatException, SymbolFormatException
  {
  ...
  /**
   * Furthermore, the mapping scrambler can be initialized with a Random
   * object, which can be used to ensure deterministic, reproducible
   * scrambling. This can be used to scramble a specific match in the same way,
   * even if it stored and reloaded in the meantime.
   */
  public void testMappingScramblerConsistency()
  {
    ...
    for (String gameKey : repo.getGameKeys())
    {
      ...
      assertFalse(aScrambledRules.toString()
          .equals(cScrambledRules.toString()));
      assertEquals(cScrambledRules.toString(), fScrambledRules.toString());
    }
  }

  private void runScramblerTest(GdlScrambler scrambler)
      throws SymbolFormatException, GdlFormatException
  {
    ...
    for (String gameKey : repo.getGameKeys())
    {
      ...
      for (Gdl rule : game.getRules())
      {
        ...
        String renderedUnscrambledRule = scrambler
            .unscramble(renderedScrambledRule).toString();
