...
{
  /**
   * Modifies a GDL description by replacing all rules in which variables could
   * be bound to functions, so that the new rules will only bind constants to
   * variables. Also automatically removes GdlOrs from the rules using the
   * DeORer. Not guaranteed to work if the GDL is written strangely, such as
   * when they include rules in which certain conjuncts are never or always
   * true. Not guaranteed to work when rules are unsafe, i.e., they contain
   * variables only appearing in the head, a negated literal, and/or a distinct
   * literal. (In fact, this can be a good way to test for GDL errors, which
      ...
      throws InterruptedException
  {
    ...
    List<Gdl> expandedRules = applyAmbiguitiesToRules(description,
                                                      ambiguitiesByOriginalForm,
                                                      model);
    return cleanUpIrrelevantRules(expandedRules);
  }

  /**
   * An ambiguity represents a particular relationship between two sentence
   * forms. It says that if sentence form "original" appears in a rule and has
   * GdlVariables in particular slots, it could be equivalent to the sentence
   * form "replacement" if functions are assigned to its variables. The goal of
   * this transformation is to make it possible for users of the game
   * description to treat it as if functions could not be assigned to
   * variables. This requires adding or modifying rules to account for the
   * extra cases.
   */
  private static class Ambiguity
  {
                                   ...
                                   SentenceForm replacement)
    {
      return new Ambiguity(original,
                           ImmutableMap
                               .copyOf(replacementsByOriginalTupleIndex),
                           replacement);
    }
    ...
    /**
     * Returns true iff the given sentence could correspond to a sentence of
     * the replacement form, for some variable assignment.
     */
    ...
    public Map<GdlVariable, GdlTerm> getReplacementAssignment(GdlSentence sentence,
                                                              UnusedVariableGenerator varGen)
    {
      ...
      for (int varIndex : replacementsByOriginalTupleIndex.keySet())
      {
        ...
        GdlFunction replacementFunction = varGen
            .replaceVariablesAndConstants(function);
  ...
  private static ListMultimap<SentenceForm, Ambiguity> getAmbiguitiesByOriginalForm(SentenceFormModel model)
      throws InterruptedException
  {
  ...
  private static ListMultimap<GdlConstant, SentenceForm> getFormsByName(SentenceFormModel model)
  {
    return Multimaps.index(getAllSentenceForms(model),
                           new Function<SentenceForm, GdlConstant>()
                           {
                             @Override
  ...
  private static List<Ambiguity> getAmbiguities(SentenceForm original,
      ...
      throws InterruptedException
  {
    ...
    Map<Integer, GdlFunction> replacementsByOriginalTupleIndex = Maps
        .newHashMap();
    //Make the arguments ?v0, ?v1, ?v2, ... so we can find the tuple indices easily
    GdlSentence originalSentence = original
        .getSentenceFromTuple(getNumberedTuple(original.getTupleSize()));
    GdlSentence replacementSentence = replacement
        .getSentenceFromTuple(getNumberedTuple(replacement.getTupleSize()));

    boolean success = findAmbiguity(originalSentence.getBody(),
                                    replacementSentence.getBody(),
                                    replacementsByOriginalTupleIndex);
    if (success)
    {
      return Optional.of(Ambiguity.create(original,
                                          replacementsByOriginalTupleIndex,
                                          replacement));
    }
    else
      ...
      else if (replacementTerm instanceof GdlFunction)
      {
        if (originalTerm instanceof GdlVariable)
        {
          int varIndex = Integer.valueOf(originalTerm.toString().replace("?v",
                                                                         ""));
          replacementsByOriginalTupleIndex.put(varIndex,
                                               (GdlFunction)replacementTerm);
        }
      ...
      throws InterruptedException
  {
    ...
    for (Gdl gdl : description)
    {
      ...
      {
        result.addAll(applyAmbiguities((GdlRule)gdl,
                                       ambiguitiesByOriginalForm,
                                       model));
      }
      else
      ...
      throws InterruptedException
  {
    ...
    for (GdlLiteral literal : Iterables.concat(ImmutableSet.of(originalRule
        .getHead()), originalRule.getBody()))
    {
      List<GdlRule> newRules = Lists.newArrayList();
      for (GdlRule rule : rules)
      {
        Preconditions.checkArgument(originalRule.arity() == rule.arity());
        newRules.addAll(applyAmbiguitiesForLiteral(literal,
                                                   rule,
                                                   ambiguitiesByOriginalForm,
                                                   model));
      }
  ...
  private static List<GdlRule> applyAmbiguitiesForLiteral(GdlLiteral literal,
                                                          GdlRule rule,
                                                          ListMultimap<SentenceForm, Ambiguity> ambiguitiesByOriginalForm,
                                                          SentenceFormModel model)
      throws InterruptedException
  {
    ...
    if (literal instanceof GdlSentence)
    {
      ...
      for (Ambiguity ambiguity : ambiguitiesByOriginalForm.get(form))
      {
        ...
        {
          Map<GdlVariable, GdlTerm> replacementAssignment = ambiguity
              .getReplacementAssignment(sentence, varGen);
          GdlRule newRule = CommonTransforms
              .replaceVariables(rule, replacementAssignment);
          results.add(newRule);
        }
      }
    }
  ...
  private static abstract class UnusedVariableGenerator
  {
    public GdlFunction replaceVariablesAndConstants(GdlFunction function)
    {
      ...
      return (GdlFunction)CommonTransforms.replaceVariables(function,
                                                            assignment);
    }
  ...
  private static UnusedVariableGenerator getVariableGenerator(final GdlRule rule)
  {
    ...
    {
      private int                    count                = 1;
      private final Set<GdlVariable> originalVarsFromRule = ImmutableSet
                                                              .copyOf(GdlUtils
                                                                  .getVariables(rule));
  ...
  private static List<Gdl> cleanUpIrrelevantRules(List<Gdl> expandedRules)
      throws InterruptedException
  {
    final ImmutableSentenceFormModel model = SentenceFormModelFactory
        .create(expandedRules);
    return ImmutableList.copyOf(Collections2.filter(expandedRules,
                                                    new Predicate<Gdl>()
                                                    {
                                                      @Override
