...
{
  /**
   * Precomputes the true sentences in every constant sentence form in the
   * given sentence model and returns the results in the form of a
   * ConstantChecker. The implementation uses a forward-chaining reasoner. For
   * accurate results, the rules used should have had the
   * {@link NewVariableConstrainer} transformation applied to them. On average,
   * this approach is more efficient than
   * {@link #createWithProver(SentenceFormModel)}.
   */
      ...
      throws InterruptedException
  {
    ...
    return ImmutableConstantChecker.create(model, Multimaps
        .filterKeys(sentencesByForm.getSentences(),
                    Predicates.in(model.getConstantSentenceForms())));
  }

  private static void addSentencesTrueByRulesDifferentially(GdlSentenceSet sentencesByForm,
                                                            SentenceDomainModel domainModel,
                                                            GdlChainingReasoner reasoner)
      throws InterruptedException
  {
    ...
    Multimap<SentenceForm, SentenceForm> dependencySubgraph = Multimaps
        .filterKeys(model.getDependencyGraph(), Predicates.in(constantForms));
    dependencySubgraph = Multimaps.filterValues(model.getDependencyGraph(),
                                                Predicates.in(constantForms));
    dependencySubgraph = ImmutableMultimap.copyOf(dependencySubgraph);
    List<Set<SentenceForm>> ordering = DependencyGraphs
        .toposortSafe(constantForms, dependencySubgraph);
    ...
    for (Set<SentenceForm> stratum : ordering)
    {
      ...
      for (SentenceForm form : stratum)
      {
        ...
        {
          GdlSentenceSet ruleResults = reasoner
              .getRuleResults(rule, domainModel, sentencesByForm);
          if (!reasoner.isSubsetOf(sentencesByForm, ruleResults))
          {
            sentencesByForm = reasoner.getUnion(sentencesByForm, ruleResults);
            newlyTrueSentences = reasoner.getUnion(newlyTrueSentences,
                                                   ruleResults);
          }
        }
      }
      ...
      while (somethingChanged)
      {
        ...
        for (SentenceForm form : stratum)
        {
          ...
          {
            GdlSentenceSet ruleResults = reasoner
                .getRuleResultsForNewSentences(rule,
                                               domainModel,
                                               sentencesByForm,
                                               newlyTrueSentences);
            if (!reasoner.isSubsetOf(sentencesByForm, ruleResults))
            {
              ...
              newStuffInThisPass = reasoner.getUnion(newStuffInThisPass,
                                                     ruleResults);
            }
          }
        }
        sentencesByForm = reasoner.getUnion(sentencesByForm,
                                            newStuffInThisPass);
   ...
   * Precomputes the true sentences in every constant sentence form in the
   * given sentence model and returns the results in the form of a
   * ConstantChecker. The implementation uses a backwards-chaining theorem
   * prover. In most (but not all) cases,
   * {@link #createWithForwardChaining(SentenceDomainModel)} is more efficient.
   */
  ...
  private static void addSentencesTrueByRules(Multimap<SentenceForm, GdlSentence> sentencesByForm,
      ...
      throws InterruptedException
  {
    ...
    {
      GdlSentence query = form.getSentenceFromTuple(getVariablesTuple(form
          .getTupleSize()));
      for (GdlSentence result : prover.askAll(query,
                                              ImmutableSet.<GdlSentence> of()))
      {
