...
public class ImmutableConstantChecker implements ConstantChecker
{
  ...
  {
    Preconditions.checkArgument(sentenceModel.getConstantSentenceForms()
        .containsAll(sentencesByForm.keySet()));
    this.sentenceModel = sentenceModel;
    this.sentencesByForm = sentencesByForm;
  }

  /**
   * Returns an ImmutableConstantChecker with contents identical to the given
   * ConstantChecker. May not actually make a copy if the input is immutable.
   */
