 ...
 * which is functionally equivalent to the original game. The AimaProver does
 * not correctly apply "distinct" literals in rules if they have not yet been
 * bound. (See test_distinct_beginning_rule.kif for an example where this comes
 * up.) The same is true for "not" literals. This transformation moves
 * "distinct" and "not" literals later in the rule, so they always appear after
 * sentence literals have defined those variables. This should be applied to
 * the input to the ProverStateMachine until this bug is fixed some other way.
 */
