...
public class StateMachineVerifier
{
  ...
  public static boolean checkMachineConsistency(StateMachine theReference,
                                                ...
                                                long timeToSpend)
  {
    ...
    GamerLogger
        .log("StateMachine", "Performing automatic consistency testing on " +
                             theSubject.getClass().getName() + " using " +
    ...
    while (true)
    {
      ...
      for (int i = 0; i < theMachines.size(); i++)
      {
        ...
        {
          GamerLogger
              .log("StateMachine", "Machine #" + i +
                                   " failed to generate an initial state!");
          return false;
        }
      }
          ...
          if (!statesMatch(theCurrentStates[i], theCurrentStates[0]))
          {
            GamerLogger.log("StateMachine",
                            "Inconsistency between machine #" + i +
                                " and ProverStateMachine over initial state " +
      ...
      while (!theMachines.get(0).isTerminal(theCurrentStates[0]))
      {
        ...
        for (int i = 1; i < theMachines.size(); i++)
        {
          for (Role theRole : theMachines.get(0).getRoles())
          {
            try
            {
              if (!(theMachines.get(i)
                  .getLegalMoves(theCurrentStates[i], theRole).size() == theMachines
                  .get(0).getLegalMoves(theCurrentStates[0], theRole).size()))
              {
                ...
                GamerLogger.log("StateMachine",
                                "Machine #" +
                                    ...
                                    theMachines
                                        .get(0)
                                        .getLegalMoves(theCurrentStates[0],
                                                       theRole).size() +
                                    " for player " + theRole);
                GamerLogger.log("StateMachine",
                                "Machine #" +
        ...
        try
        {
          ...
          List<Move> theJointMove = theMachines.get(0)
              .getRandomJointMove(theCurrentStates[0]);
          ...
          for (int i = 0; i < theMachines.size(); i++)
          {
            try
            {
              theCurrentStates[i] = theMachines.get(i)
                  .getNextState(theCurrentStates[i], theJointMove);
            }
            catch (Exception e)
            {
              GamerLogger.logStackTrace("StateMachine", e);
            ...
            if (matchFullStates && i > 0)
            {
              if (!statesMatch(theCurrentStates[i], theCurrentStates[0]))
              {
                GamerLogger.log("StateMachine",
                                "Inconsistency between machine #" + i +
                                    " and ProverStateMachine after move " +
                                    theJointMove + " : " +
                                    theCurrentStates[0] + " vs " +
                                    theCurrentStates[i].getContents());
                return false;
      ...
      for (int i = 1; i < theMachines.size(); i++)
      {
        if (!theMachines.get(i).isTerminal(theCurrentStates[i]))
        {
          GamerLogger
              .log("StateMachine",
                   "Inconsistency between machine #" + i +
                       " and ProverStateMachine over terminal-ness of state " +
                       theCurrentStates[0] + " vs " + theCurrentStates[i]);
          System.out.println(theMachines.get(i)
              .isTerminal(theCurrentStates[i]));
          return false;
        }
        for (Role theRole : theMachines.get(0).getRoles())
        {
          ...
          try
          {
            while (true)
            {
              if (theMachines.get(i).getGoal(theCurrentStates[i], theRole) != theMachines
                  .get(0).getGoal(theCurrentStates[0], theRole))
              {
                GamerLogger
                    .log("StateMachine",
                         "Inconsistency between machine #" +
                             i +
                             " and ProverStateMachine over goal value for " +
                             theRole +
                             " of state " +
                             theCurrentStates[0] +
                             ": " +
                             theMachines.get(i).getGoal(theCurrentStates[i],
                                                        theRole) +
                             " vs " +
                             theMachines.get(0).getGoal(theCurrentStates[0],
                                                        theRole));
                return false;
              }
              break;
          ...
          catch (Exception e)
          {
            GamerLogger
                .log("StateMachine",
                     "Inconsistency between machine #" + i +
                         " and ProverStateMachine over goal-ness of state " +
                         theCurrentStates[0] + " vs " + theCurrentStates[i]);
            GamerLogger.logStackTrace("StateMachine", e);
            return false;
          }
    ...
    GamerLogger.log("StateMachine",
                    "Completed automatic consistency testing on " +
                        theSubject.getClass().getName() + ", w/ " + nRound +
                        " rounds: all tests pass!");
    return true;
  }
}