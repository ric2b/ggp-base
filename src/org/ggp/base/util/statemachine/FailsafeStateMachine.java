...
/**
 * The FailsafeStateMachine is a wrapper around a particular state machine. It
 * will catch errors/exceptions being thrown from that state machine, and fall
...
public class FailsafeStateMachine extends StateMachine
{
  ...
  public synchronized void initialize(List<Gdl> description)
  {
    ...
    GamerLogger
        .logError("StateMachine",
                  "Failsafe Machine: failed to load initial state machine. Falling back...");
    if (attemptLoadingProverMachine())
      return;

    GamerLogger
        .logError("StateMachine",
                  "Failsafe Machine: catastrophic failure to load *any* state machine. Cannot recover.");
    GamerLogger
        .logError("StateMachine",
                  "Failsafe Machine: cannot recover from current state. Shutting down.");
  ...
  private void failGracefully(Exception e1, Error e2)
  {
    ...
    GamerLogger
        .logError("StateMachine",
                  "Failsafe Machine: graceful failure mode kicking in.");

    if (theBackingMachine.getClass() != ProverStateMachine.class)
    {
      GamerLogger
          .logError("StateMachine", "Failsafe Machine: online failure for " +
                                    theBackingMachine.getClass() +
    ...
    GamerLogger
        .logError("StateMachine",
                  "Failsafe Machine: online failure for regular prover. Cannot recover.");
  }
  ...
  private boolean attemptLoadingInitialMachine()
  {
    try
    {
      ...
      GamerLogger
          .log("StateMachine",
               "Failsafe Machine: successfully activated initial state machine for use!");
      return true;
    }
    catch (Exception e1)
    {
  ...
  private boolean attemptLoadingProverMachine()
  {
    try
    {
      ...
      GamerLogger
          .log("StateMachine",
               "Failsafe Machine: successfully loaded traditional prover.");
      return true;
    }
    catch (Exception e1)
    {
  ...
  @Override
  public MachineState performDepthCharge(MachineState state, int[] theDepth)
  ...
  @Override
  public void getAverageDiscountedScoresFromRepeatedDepthCharges(MachineState state,
                                                                 double[] avgScores,
                                                                 double[] avgDepth,
      ...
      GoalDefinitionException
  {
    ...
    {
      theBackingMachine
          .getAverageDiscountedScoresFromRepeatedDepthCharges(state,
                                                              avgScores,
                                                              avgDepth,
                                                              discountFactor,
                                                              repetitions);
