...
/**
 * This is a generic implementation of a Time-To-Live cache that maps keys of
 * type K to values of type V. It's backed by a hashmap, and whenever a pair
 * (K,V) is accessed, their TTL is reset to the starting TTL (which is the
 * parameter passed to the constructor). On the other hand, when the method
 * prune() is called, the TTL of all of the pairs in the map is decremented,
 * and pairs whose TTL has reached zero are removed. While this class
 * implements the Map interface, keep in mind that it only decrements the TTL
 * of an entry when that entry is accessed directly.
 * 
 * @param <K>
 *          Key type
 * @param <V>
