...
/**
 * GameStateRenderer generates an image that represents the current state of a
 * match, based on the current state of the match (in XML) and an XSLT that
 * converts that XML match state into HTML. After rendering the match state in
 * HTML as a DOM, it renders that DOM into a BufferedImage which can be
...
public class GameStateRenderer
{
  ...
  public static synchronized void renderImagefromGameXML(String gameXML,
                                                         String XSL,
  ...
  {
    XSL = XSL
        .replace("<!DOCTYPE stylesheet [<!ENTITY ROOT \"http://games.ggp.org/base\">]>",
                 "");
    ...
    try
    {
      ...
      Transformer transformer = tFactory
          .newTransformer(new StreamSource(xslIOString.getInputStream()));
