 ...
 * disk, bundled with the GGP Base project. For consistency with the web-based
...
public final class LocalGameRepository extends GameRepository
{
  ...
  private static String               theLocalRepoURL    = "http://127.0.0.1:" +
                                                           REPO_SERVER_PORT;
  ...
  static class BaseRepository
  {
    ...
    public static byte[] getResponseBytesForURI(String reqURI)
        throws IOException
    {
      ...
      if (reqURI.equals("/games/metadata"))
      {
        ...
        {
          if (shouldIgnoreFile(gameName))
            continue;
          try
          {
            theGameMetaMap
                .put(gameName,
                     new JSONObject(new String(getResponseBytesForURI("/games/" +
                                                                      gameName +
                                                                      "/"))));
          }
      ...
      if (theExplicitVersion == null)
      {
        ...
        {
          throw new RuntimeException(reqURI + " != [~v] " +
                                     (thePrefix + "/" + theSuffix));
        }
      }
      else
      {
        if (!reqURI.equals(thePrefix + "/v" + theExplicitVersion + "/" +
                           theSuffix))
        {
          throw new RuntimeException(reqURI +
                                     " != [v] " +
                                     (thePrefix + "/v" + theExplicitVersion +
                                      "/" + theSuffix));
        }
      }
      ...
      {
        byte[] theBytes = getBytesForVersionedFile(thePrefix,
                                                   theFetchedVersion,
                                                   theSuffix);
        if (theBytes != null)
        {
          ...
          {
            theBytes = adjustMetadataJSON(reqURI,
                                          theBytes,
                                          theExplicitVersion,
                                          nMaxVersion);
          }
    ...
    public static byte[] adjustMetadataJSON(String reqURI,
                                            byte[] theMetaBytes,
                                            Integer nExplicitVersion,
                                            int nMaxVersion)
        throws IOException
    {
      try
      {
        ...
        String theRulesheet = new String(getResponseBytesForURI(reqURI
            .replace("METADATA", theMetaJSON.getString("rulesheet"))));
        MetadataCompleter.completeMetadataFromRulesheet(theMetaJSON,
                                                        theRulesheet);
    ...
    private static byte[] getBytesForVersionedFile(String thePrefix,
                                                   int theVersion,
                                                   String theSuffix)
    {
      if (theVersion == 0)
    ...
    private static String transformJS(String theContent) throws IOException
    {
      ...
      if (theContent.contains("[BOARD_INTERFACE_JS]"))
      {
        ...
        theContent = theContent.replaceFirst("\\[BOARD_INTERFACE_JS\\]",
                                             theCommonBoardJS);
      }
      return theContent;
    }
    ...
    private static String readDirectory(File theDirectory)
    {
      ...
      {
        if (shouldIgnoreFile(children[i]))
          continue;
  ...
  {
    /**
     * Complete fields in the metadata procedurally, based on the game
     * rulesheet. This is used to fill in the number of roles, and create a
     ...
     */
    public static void completeMetadataFromRulesheet(JSONObject theMetaJSON,
                                                     String theRulesheet)
        throws JSONException
    {
