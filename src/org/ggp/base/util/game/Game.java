 ...
 * metadata, like the game's name and its associated game repository URL. Games
 * do not necessarily have all of these fields. Games loaded from local storage
 * will not have a repository URL, and probably will be missing other metadata
 * as well. Games sent over the wire from a game server rather than loaded from
 * a repository are called "ephemeral" games, and contain only their rulesheet;
 * they have no metadata, and do not even have unique keys. Aside from
 * ephemeral games, all games have a key that is unique within their containing
 * repository (either local storage or a remote repository). Games can be
 * indexed internally using this key. Whenever possible, the user should be
 * shown the game's name (if available) rather than the internal key, since the
 * game's name is more readable/informative than the key. (e.g. A game with the
 * name "Three-Player Free-For-All" but the key "3pffa".) NOTE: Games are
 * different from matches. Games represent the actual game being played,
 * whereas matches are particular instances in which players played through the
 * game. For example, you might have a Game object that contains information
 * about chess: it would contain the rules for chess, methods for visualizing
 * chess matches, a human readable description of the rules of chess, and so
 * on. On the other hand, for any particular chess match between two players,
 * you would have a Match object that has a record of what moves were played,
 * what states were transitioned through, when everything happened, how the
 * match was configured, and so on. There can be many Match objects all
 * associated with a single Game object, just as there can be many matches
 * played of a particular game. NOTE: Games operate only on "processed"
 * rulesheets, which have been stripped of comments and are properly formatted
 * as SymbolLists. Rulesheets which have not been processed in this fashion
...
public final class Game
{
   ...
   * comments and adding opening and closing parens so that the rulesheet is a
   * valid SymbolList. This must be done to any raw rulesheets coming from the
  ...
  /**
   * Gets the GDL object representation of the game rulesheet. This
   * representation is generated when "getRules" is called, rather than when
   * the game is created, so that it's safe to drain the GDL pool between when
   * the game repository is loaded and when the games are actually used. This
   * doesn't incur a performance penalty because this method is usually called
