 ...
 * details include: > Locations of the "java" and "javac" binaries, independent
 * of OS. > Amounts of RAM to allocate to the gamer when running in Proxy mode,
 * based on the name of the system it's running on. Machine-specific profiles
 * are stored in an optional "gamerProfiles" file, which has the following
 * format: (system name) <tab> (allocated RAM in MB) <tab> (proper name) When
 * the program begins, GamerConfiguration will automatically determine which
 * profile is applicable, and which operating system is running. From then on,
...
public class GamerConfiguration
{
  ...
  public static void showConfiguration()
  {
    ...
    System.out
        .println(String
            .format("Configured according to the %s Profile, running on %s which is a variety of %s, allocating %d MB of memory to the gaming process.",
                    strProfileName,
                    strSystemOS,
                    osType,
                    nMemoryForGamer));
